...
3. **Sign in (PKCE).** The client calls `POST /v2/login` with the email, the
   `password` (the derived _server password_, not the user's password), and the
...
- **Trusted devices (Standard Red Notes)** — a per-device token that bypasses
  _only_ the 2FA gate (never the account password) on future sign-ins. See
...
{% include safety-alert.html
  level="danger"
  title="A full MCP token is a decrypting credential"
  body="The full bridge token can unwrap items keys and decrypt notes without the account password. The server enforces read-only versus write mode, but the current bridge does not enforce scopeTagUuids. Deleting the token prevents new authentication; it does not invalidate a session already issued from the token or erase the bridge's local data. Follow the complete compromise procedure, not token deletion alone."
  link_url="/mcp-bridge.html#scope-and-revocation-limits"
  link_text="Use the MCP compromise procedure"
%}
