(() => {
  ...
  const THEME_STORAGE_KEY = "standard-red-notes-docs-theme";
  const themeRoot = document.documentElement;
  const themeToggle = document.querySelector("[data-docs-theme-toggle]");
  const themeIcon = themeToggle?.querySelector("[data-docs-theme-icon]");
  const themeLabel = themeToggle?.querySelector("[data-docs-theme-label]");
  const themeColor = document.querySelector("[data-docs-theme-color]");
  ...
  const applyTheme = (
    ...
    if (notify) {
      window.dispatchEvent(
        new CustomEvent("srn:themechange", { detail: { theme } }),
      );
    }
  };

  applyTheme(storedTheme ?? themeRoot.dataset.theme);
  ...
  const setSidebarOpen = (open) => {
    ...
    sidebarToggle.querySelector("[data-docs-nav-toggle-label]").textContent =
      open ? "Close docs" : "Browse docs";
  };
  ...
  const activateCurrentNavigationLink = () => {
    ...
    if (!currentLink && currentHash) {
      currentLink = links.find((link) => {
        const target = new URL(link.href, window.location.href);
        return (
          normalizePathname(target.pathname) === currentPath && !target.hash
        );
      });
  ...
  const articleHeadings = [
    ...document.querySelectorAll(
      ".docs-content article h2, .docs-content article h3, .docs-content article h4",
    ),
  ];
  ...
  if (pageTableOfContents && articleHeadings.length > 1) {
    ...
    for (const heading of articleHeadings) {
      const explicitAnchor =
        heading.previousElementSibling?.tagName === "A"
          ? heading.previousElementSibling.getAttribute("id")
          : null;
    ...
    if ("IntersectionObserver" in window) {
      ...
      const observer = new IntersectionObserver(
        (entries) => {
          ...
          const nearest = [...visibleHeadings.entries()].sort(
      ...
      try {
        ...
        if (navigator.clipboard?.writeText && window.isSecureContext) {
          ...
          const previousRanges = selection
            ? [...Array(selection.rangeCount)].map((_, index) =>
