...
| Push bridge          | `redis`, `in-process`, or `none`, with readiness | Both `redis` and `in-process` count as bound and healthy; a single process that holds every socket needs no Redis to deliver a push. Only `none` is a fault, and it has two causes: a process asked for a Redis-backed plane with no reachable host, or a server build older than the in-process plane, which needs an upgrade rather than a setting. Bound but not ready is a reconnect window that recovers on its own. |
...
- **The socket is attached with no push bridge.** The lane accepts clients, but
  ...
  no Redis reports an in-process bridge instead and is healthy, and a
  multi-container one reports `redis`; only `none` is a fault. The other way to
  reach it is a server build older than the in-process plane, which an upgrade
  fixes rather than any setting.
