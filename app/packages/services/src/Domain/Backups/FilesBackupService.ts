  ...
  implements BackupServiceInterface, InternalEventHandlerInterface
{
    ...
    for (const file of files) {
      ...
      if (record == undefined) {
        ...
        try {
          ...
          /**
           * Encryption, token creation, networking, and native disk writes can
           * all fail. Never leave a file permanently marked as pending: a
           * subsequent observer pass must be able to retry it.
           */
  ...
  private async performBackupOperation(file: FileItem): Promise<'success' | 'failed' | 'aborted'> {
    ...
    try {
      ...
      const downloadType =
        !file.user_uuid || file.user_uuid === this.session.getSureUser().uuid ? 'user' : 'shared-vault'

      result = await this.device.saveFilesBackupsFile(location, file.uuid, metaFileAsString, {
      ...
      console.error(`Failed to back up file ${file.uuid}`, message)
      return 'failed'
    } finally {
