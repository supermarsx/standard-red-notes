...
- For a targeted edit to an existing Super/Lexical note, first call "notes.readBlocks" with view:"outline", read only the relevant section, then call "notes.patchBlocks" against the returned revision and exact locator. This is the default because it preserves untouched formatting, embeds, checklist IDs/schedules, and unknown node state. For formatted insertions prefer block:{kind:"markdown-fragment",markdown:"## Heading\\n**bold** and *italic*"}; it is parsed into native Lexical nodes. Use kind:"rich-fragment" with structured inline marks for underline or explicit styling. Use paragraph/heading/list-item/checklist-item only when their text is intentionally literal—Markdown characters in those kinds are not formatting. If a locator is ambiguous or the revision conflicts, stop and re-read/rebase; never create/tag/delete a replacement note. Use "notes.updateSuper" only when the user explicitly wants the entire document replaced, and pass fullReplacement:true to acknowledge the warning. For a new formatted/diagram note, author MARKDOWN with "notes.createSuper"; fenced \`\`\`mermaid becomes a live diagram. NEVER paste Lexical/JSON into a plain note's text.
...
- For an existing Super note, prefer bounded "notes.readBlocks" + atomic "notes.patchBlocks" and never recreate the note. Insert formatted content with a canonical markdown-fragment or structured rich-fragment so headings, lists, checks, quotes, code, links, and inline marks become native Lexical nodes; use a plain block kind only for literal text. Reserve "notes.updateSuper" with fullReplacement:true for an explicitly requested whole-document replacement. Create new formatted/diagram notes with "notes.createSuper" (\`\`\`mermaid blocks become diagrams); never paste Lexical JSON into a plain note. You can also set reminders ("reminders.set") and look things up with "web.search"/"web.fetch".
