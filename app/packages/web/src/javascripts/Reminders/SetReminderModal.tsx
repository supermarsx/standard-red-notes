...
import { getNotificationPermission, requestNotificationPermission, notificationsSupported } from './notificationService'
import {
  createEmailReminder,
  deleteEmailReminder,
...
const SetReminderModalContent = observer(({ application, notesController, note, close }: Omit<Props, 'isOpen'>) => {
  ...
  const [dueLocal, setDueLocal] = useState<string>(() => {
    return editing ? isoToLocalInputValue(editing.dueAt) : defaultLocalInputValue()
  })
  ...
  const editingRecurrence = useMemo(() => normalizeRecurrence(editing?.recurrence), [editing])
  ...
  const enableNotifications = useCallback(async () => {
    ...
    if (result === 'denied') {
      addToast({
        ...
        message: 'OS notifications are blocked. Reminders will still appear inside the app as toasts.',
      })
    }
  }, [])
    ...
    >
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-1">
          ...
          />
          {previewIso && (
            <span className="text-passive-0 text-xs">{formatReminderRelative({ id: '', dueAt: previewIso }, now)}</span>
          )}
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          {frequency === 'custom' && (
            <div className="mt-1 flex items-center gap-2">
              <span className="text-passive-0 text-sm">Every</span>
              <input
                type="number"
                min={1}
                className="border-border bg-default w-16 rounded border px-2 py-1.5 text-sm"
          ...
          {recurrenceSummary && <span className="text-passive-0 text-xs">{recurrenceSummary}</span>}
        </div>
        ...
        {hasAccount && (
          <div className="border-border flex flex-col gap-1 rounded border p-3">
            <label className="flex items-center gap-2 text-sm font-semibold">
              <input type="checkbox" checked={emailMe} onChange={(event) => setEmailMe(event.target.checked)} />
              Also email me this reminder
            </label>
            <p className="text-passive-0 text-xs">
              Emailing a reminder sends its time and text to the server in plaintext (it leaves end-to-end encryption)
              so it can be emailed to you. Only this reminder is shared; your note stays encrypted. Email is sent only
              if your server operator has enabled and configured email reminders. You can cancel it any time from
              Preferences &rsaquo; Email reminders, or by clearing this reminder.
            </p>
          </div>
        )}
        ...
        {notificationsSupported() ? (
          permission === 'granted' ? (
            <p className="text-passive-0 text-xs">
              Desktop notifications are enabled. You&rsquo;ll also see an in-app alert when this reminder is due.
            </p>
          ) : permission === 'denied' ? (
            <p className="text-passive-0 text-xs">
              Desktop notifications are blocked by your browser. Reminders will still appear inside the app.
            </p>
          ) : (
            <button
              type="button"
              onClick={() => void enableNotifications()}
              className="border-border text-info self-start rounded border px-3 py-1.5 text-sm hover:underline"
            >
        ...
        {editing && (
          <button
            type="button"
            onClick={() => void clear()}
            className="text-danger self-start rounded px-2 py-1.5 text-sm hover:underline"
          >
            Clear reminder
          </button>
        )}
...
const SetReminderModal = ({ application, notesController, note, isOpen, close }: Props) => {
  return (
    <ModalOverlay isOpen={isOpen} close={close} className="md:max-w-[30rem]">
      <SetReminderModalContent application={application} notesController={notesController} note={note} close={close} />
    </ModalOverlay>
  )
}
