...
const MoveFileToFolderOption: FunctionComponent<Props> = ({ navigationController, file, iconClassName, disabled }) => {
  ...
  const currentFolder = navigationController.getFileFolder(file)

  const moveToFolder = useCallback(
    (folder: SNFolder | undefined) => {
      navigationController.moveFileToFolder(file, folder).catch(console.error)
      setIsOpen(false)
    },
    [navigationController, file],
  )
...
export default observer(MoveFileToFolderOption)
