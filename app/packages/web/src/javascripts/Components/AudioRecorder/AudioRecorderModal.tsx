...
const AudioRecorderContent = observer(({ application, filesController, note, close }: Omit<Props, 'isOpen'>) => {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        {!supported && (
          <div className="border-danger bg-danger-faded rounded border border-solid p-3 text-sm">
        ...
        {canTranscribe && (
          <div className="border-warning bg-warning-faded rounded border border-solid p-3 text-sm">
            <div className="text-warning font-semibold">Transcription sends audio to your AI endpoint</div>
            <p className="mt-1">
              ...
              ) for speech-to-text. Saving the recording to the note does not send it anywhere except your own Standard
              Red Notes file storage.
            </p>
          </div>
        )}

        {/* Recorder controls */}
        <div className="border-border flex flex-col items-center gap-3 rounded border p-4">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            ...
            {phase === 'recording' && (
              <>
                <button
                  className="border-border flex items-center gap-1 rounded border px-3 py-1.5 text-sm"
                ...
                <button
                  className="bg-danger text-danger-contrast flex items-center gap-1 rounded px-3 py-1.5 text-sm font-semibold"
            ...
            {phase === 'paused' && (
              <>
                <button
                  className="bg-info text-info-contrast flex items-center gap-1 rounded px-3 py-1.5 text-sm font-semibold"
          ...
          {permissionError && <p className="text-danger text-sm">{permissionError}</p>}
        </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            {!canTranscribe && (
              <p className="text-passive-0 text-xs">
                Recorded-audio transcription needs a Direct-mode AI endpoint with a <code>/audio/transcriptions</code>{' '}
                route. For on-device speech-to-text, use live Dictation instead (Chromium browsers).
              </p>
            )}
            {transcribeError && <p className="text-danger text-sm">Could not transcribe: {transcribeError}</p>}
          </div>
        )}
        ...
        {(transcript || transcribing) && (
          <div className="flex flex-col gap-1">
            <div className="flex items-center justify-between">
              ...
              <button className="text-info text-xs hover:underline" onClick={() => void copyTranscript()}>
                Copy
              </button>
            </div>
            <textarea
            ...
            <div>
              <button
                className="bg-info text-info-contrast mt-1 flex items-center gap-1 rounded px-3 py-1.5 text-sm font-semibold disabled:opacity-50"
