 ...
 * "Edit styles" button (opens the P3 modal) sits on the same line, plus an
 * optional overflow menu for extra actions.
 */
...
/** Optional "⋮" overflow menu hosting extra (non-block-type) actions. */
const ExtraMenu = ({ children }: { children: ReactNode }) => {
  const [open, setOpen] = useState(false)
  const anchorRef = useRef<HTMLButtonElement>(null)
  return (
    <>
      <button
        ...
        onMouseDown={(event) => event.preventDefault()}
        className={classNames(
          'flex flex-shrink-0 items-center justify-center rounded border border-border bg-default px-1.5 py-1',
          'text-passive-0 transition-colors duration-75 hover:border-info hover:bg-contrast focus:outline-none focus-visible:border-info',
          open ? 'border-info bg-contrast' : '',
        )}
      >
        <Icon type="more" size="custom" className="h-4 w-4" />
      </button>
      <Popover
        title="More options"
        anchorElement={anchorRef}
        ...
        disableMobileFullscreenTakeover
        disableFlip
        containerClassName="md:!min-w-56 md:!w-auto"
      >
 ...
 * P3 modal) — and, optionally, an "⋮" menu of extra actions — reachable even at
 * very narrow widths. Never causes horizontal overflow.
 */
export const BlockStyleGalleryBar = ({
  ...
  onEditStyles: () => void
  /** Optional menu content rendered in a trailing "⋮" overflow menu. */
  extraMenu?: ReactNode
}) => {
  // The flex-grow track the squares live in; its width drives the fit math. It
  // fills the space left by the Edit-styles button + optional "⋮" menu, so those
  // are naturally excluded from the measurement.
  ...
  return (
    <div className="flex w-full min-w-0 items-stretch gap-1.5" onMouseDown={(event) => event.preventDefault()}>
      ...
      {extraMenu ? <ExtraMenu>{extraMenu}</ExtraMenu> : null}
    </div>
  )
}
