...
function KanbanComponent({ data, nodeKey }: { data: KanbanData; nodeKey: NodeKey }): React.JSX.Element {
  ...
  const removeCard = (colId: string, cardId: string) =>
    mutate((d) => {
      const col = d.columns.find((c) => c.id === colId)
      if (col) {
        col.cards = col.cards.filter((cc) => cc.id !== cardId)
      }
    })
  const moveCard = (colId: string, cardId: string, dir: -1 | 1) =>
    mutate((d) => {
      ...
      const to = from + dir
      if (from < 0 || to < 0 || to >= d.columns.length) {
        return
      }
  ...
  return (
    <div className="border-border bg-default my-2 rounded border" data-kanban-block="true">
      <div className="border-border text-passive-1 flex items-center justify-between gap-2 border-b px-2 py-1 text-xs">
        ...
        />
        <button className="hover:bg-contrast flex-shrink-0 rounded px-2 py-0.5" onClick={addColumn} type="button">
          + Column
        </button>
      </div>
      <div className="flex gap-2 overflow-x-auto p-2">
        {data.columns.map((col, colIndex) => (
          <div
            key={col.id}
            className="bg-contrast flex w-56 flex-shrink-0 flex-col overflow-hidden rounded"
          ...
          >
            ...
            >
              <div className="flex items-center gap-1">
                <input
                  key={`title-${col.id}`}
                  className="text-text min-w-0 flex-grow bg-transparent text-sm font-semibold outline-none"
              ...
              <div className="flex flex-wrap items-center gap-1">
                <label
                  className="border-border relative h-4 w-4 flex-shrink-0 cursor-pointer rounded-full border"
            ...
            <div className="flex flex-col p-2 pt-0">
              <div className="flex flex-col gap-1">
                {col.cards.map((card) => (
                  <div key={card.id} className="border-border bg-default rounded border p-1.5">
                    <textarea
                      key={`text-${card.id}`}
                      className="text-foreground w-full resize-none bg-transparent text-sm outline-none"
              ...
              <button
                className="text-passive-1 hover:bg-default mt-1 rounded px-1 py-0.5 text-left text-xs"
