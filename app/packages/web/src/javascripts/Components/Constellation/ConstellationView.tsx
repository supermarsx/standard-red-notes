...
import { ConstellationScope, ConstellationScopeKind, selectConstellationNoteUuids } from './selectConstellationNotes'
import {
...
function collectionNoteUuids(application: WebApplication, collection: DecryptedItemInterface): string[] {
  return application.items.referencesForItem<SNNote>(collection, ContentType.TYPES.Note).map((note) => note.uuid)
}
...
const ConstellationView = forwardRef<HTMLDivElement, Props>(
  ({ application, className, id, children, standalone }, ref) => {
    ...
    const activeNoteUuid =
      activeNote && activeNote.content_type === ContentType.TYPES.Note ? activeNote.uuid : undefined
    ...
    const folders = useMemo(() => application.items.getItems<SNFolder>(FolderContentType), [application, scopeKind])

    const scope: ConstellationScope = useMemo(() => ({ kind: scopeKind, collectionUuid }), [scopeKind, collectionUuid])
    ...
    return (
      <div
        ...
        className={classNames(
          className,
          'bg-default flex h-full flex-col overflow-hidden',
          standalone
            ? ''
            : position === 'left'
      ...
      >
        <div className="border-border bg-contrast flex items-center justify-between border-b px-4 py-3">
          <div className="flex min-w-0 items-center gap-2">
            <Icon type="star-filled" className="text-info flex-shrink-0" />
            <span className="text-base font-bold">Constellation</span>
            <span className="text-neutral ml-1 truncate text-xs">
        ...
        <div className="border-border bg-contrast flex flex-wrap items-center gap-2 border-b px-3 py-2">
          <div
            role="tablist"
            aria-label="Constellation scope"
            className="bg-default flex flex-wrap gap-0.5 rounded p-0.5"
          >
            {(
              [
          ...
          {(scopeKind === 'tag' || scopeKind === 'folder') && (
            <select
              className="border-border bg-default text-text max-w-[12rem] min-w-0 flex-grow truncate rounded border px-2 py-1 text-xs sm:flex-grow-0"
        ...
        <div ref={containerRef} className="relative flex-grow overflow-hidden">
          ...
          {counts.notes > 0 && (
            <div className="border-border bg-default/90 shadow-main absolute right-3 bottom-3 z-10 flex flex-col gap-0.5 rounded-md border p-0.5 backdrop-blur-sm">
              <button
                className="hover:bg-contrast rounded p-1.5"
          ...
          {counts.notes === 0 && (
            <div className="text-neutral pointer-events-none absolute inset-0 flex items-center justify-center px-6 text-center text-sm">
              {scopeKind === 'current'
                ? activeNoteUuid
          ...
          {counts.notes > 0 && counts.links === 0 && (
            <div className="text-neutral pointer-events-none absolute right-0 bottom-3 left-0 px-6 text-center text-xs">
              No links yet — link notes from a note’s options to connect them.
            </div>
          )}
          {selected && (
            <div
              className="border-border bg-default shadow-main absolute z-10 w-64 max-w-[85%] overflow-hidden rounded-md border"
              style={{
            ...
            >
              <div className="border-border bg-contrast flex items-start justify-between gap-2 border-b px-3 py-2">
                <span className="text-text text-sm font-semibold break-words">{selected.title}</span>
                <button
              ...
              <div className="text-neutral max-h-24 overflow-y-auto px-3 py-2 text-xs">
                {selected.summary ? (
                  <p className="break-words whitespace-pre-wrap">{selected.summary}</p>
                ) : (
              ...
              <div className="border-border border-t px-3 py-2">
                <button
                  className="bg-info text-info-contrast w-full rounded px-2 py-1 text-sm font-semibold hover:opacity-90"
                  onClick={() => {
