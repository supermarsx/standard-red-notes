...
import EmailDeliveryControlPlane, { EmailDeliveryControlPlaneAvailability } from './EmailDeliveryControlPlane'
...
const AdminEmailDeliveryTab: FunctionComponent<Props> = ({
  ...
  const [controlPlaneAvailability, setControlPlaneAvailability] =
    useState<EmailDeliveryControlPlaneAvailability>('probing')

  const handleControlPlaneAvailability = useCallback((availability: EmailDeliveryControlPlaneAvailability) => {
    setControlPlaneAvailability(availability)
  }, [])
  ...
  return (
    <PreferencesSegment>
      ...
      <Text>
        Configure outbound email for sign-in and account messages, email backups, and published reminders. Supported
        servers can route through ordered SMTP and provider API profiles; compatible single-server deployments fall back
        to one SMTP connection. Saved changes take effect on the next send.
      </Text>

      <EmailDeliveryControlPlane
        application={application}
        noteIfForbidden={noteIfForbidden}
        onAvailabilityChange={handleControlPlaneAvailability}
      />

      {controlPlaneAvailability === 'unavailable' ? (
        <div className="border-border mt-7 border-t pt-5">
          <Subtitle>Compatible single-SMTP configuration</Subtitle>
          <Text className="mt-1 text-xs">
            This fallback is used only because the server does not expose relay-profile management. Values saved here
            override matching environment settings.
          </Text>
          ...
          {loading ? (
            ...
            <Text className="mt-3">
              Email delivery settings are unavailable on this server. Update the server first.
            </Text>
          ) : error ? (
            <div className="mt-3">
            ...
            <div className="mt-4 flex max-w-3xl flex-col gap-4">
              ...
              >
                ...
                <Text className="mt-1 text-xs">
                  Password: {emailDelivery?.passwordConfigured ? 'configured (write-only)' : 'not configured'}.
                  Passwords and raw provider errors are never returned to this page.
                </Text>
              </div>
              ...
              <div>
                <Button
                  label={saving ? 'Saving…' : 'Save email delivery'}
                  onClick={() => void save()}
                  disabled={saving}
                />
              </div>
