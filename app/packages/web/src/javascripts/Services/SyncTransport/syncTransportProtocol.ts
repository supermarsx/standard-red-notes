 ...
 * do about it. Canonical here, beside the type it keys, because two consumers need
 * the same wording: the admin Diagnostics tab renders it, and the transport logs it
 * to the console when saves leave the socket (the only signal a non-admin gets).
 *
 * SECURITY: wording only. Nothing here may name a configured VALUE — an environment
 * variable's name is fine, its contents never are.
 */
export const SYNC_FALLBACK_REASON_EXPLANATIONS: Record<SyncFallbackReason, string> = {
