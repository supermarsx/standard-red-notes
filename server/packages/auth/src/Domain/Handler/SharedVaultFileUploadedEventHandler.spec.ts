...
describe('SharedVaultFileUploadedEventHandler', () => {
  ...
  const event = {
    payload: { vaultOwnerUuid, uploaderUuid: 'someone-else', fileByteSize: 500 },
  } as unknown as jest.Mocked<SharedVaultFileUploadedEvent>

  const createHandler = () => new SharedVaultFileUploadedEventHandler(updateStorageQuotaUsedForUser, logger)
  ...
  it('should charge the upload to the vault owner, not the uploader', async () => {
  ...
  it('should log an error if the quota update fails', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith('Failed to update storage quota after a shared-vault file was uploaded.')
  })
})
