...
describe('SubscriptionExpiredEventHandler', () => {
  ...
  const eventWith = (payload: Record<string, unknown>) =>
    ({ payload }) as unknown as jest.Mocked<SubscriptionExpiredEvent>
  ...
  it('should expire the offline subscription without touching roles', async () => {
  ...
  it('should not expire anything if the user is not found', async () => {
  ...
  it('should expire the subscription and remove the role from every subscription user', async () => {
    await createHandler().handle(eventWith(fullPayload))

    expect(userSubscriptionRepository.updateEndsAt).toHaveBeenCalledWith(subscriptionId, timestamp, timestamp)
    expect(userSubscriptionRepository.findBySubscriptionId).toHaveBeenCalledWith(subscriptionId)
