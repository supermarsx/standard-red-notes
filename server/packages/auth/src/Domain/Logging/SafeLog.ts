...
export function safeRequestLogMetadata(metadata: {
  method?: unknown
  url?: unknown
  userId?: unknown
}): Record<string, unknown> {
  const rawMethod = safeDataProperty(metadata, 'method')
  const rawUserId = safeDataProperty(metadata, 'userId')
  ...
  return {
    method,
    url: sanitizeUrlForSafeLog(safeDataProperty(metadata, 'url')),
    userId,
  }
}
