...
export class AddUserBanned1718800000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query('ALTER TABLE `users` ADD `banned` tinyint(1) NOT NULL DEFAULT 0')
    await queryRunner.query('ALTER TABLE `users` ADD `banned_at` datetime NULL')
    await queryRunner.query('ALTER TABLE `users` ADD `ban_reason` varchar(255) NULL')
  }
