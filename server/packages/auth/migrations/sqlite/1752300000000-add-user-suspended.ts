...
export class AddUserSuspended1752300000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query('ALTER TABLE `users` ADD `suspended` boolean NOT NULL DEFAULT (0)')
    await queryRunner.query('ALTER TABLE `users` ADD `suspended_at` datetime')
    await queryRunner.query('ALTER TABLE `users` ADD `suspended_reason` varchar(255)')
  }
