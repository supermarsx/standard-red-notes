...
describe('SharedVaultFileRemovedEventHandler', () => {
  ...
  const createHandler = () =>
    new SharedVaultFileRemovedEventHandler(updateStorageQuotaUsedInSharedVault, addNotificationsForUsers, logger)

  const event = (uuid = sharedVaultUuid) =>
    ({
      payload: { sharedVaultUuid: uuid, fileByteSize: 2048 },
    }) as jest.Mocked<SharedVaultFileRemovedEvent>
  ...
  it('subtracts the removed file size from the shared vault quota', async () => {
    await createHandler().handle(event())

    expect(updateStorageQuotaUsedInSharedVault.execute).toHaveBeenCalledWith({ sharedVaultUuid, bytesUsed: -2048 })
  })

  it('notifies the vault members that a file was removed', async () => {
    await createHandler().handle(event())
