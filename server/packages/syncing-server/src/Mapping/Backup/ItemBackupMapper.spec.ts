...
import { ItemBackupMapper } from './ItemBackupMapper'

describe('ItemBackupMapper', () => {
  ...
  const createMapper = () => new ItemBackupMapper(timer)
  ...
  it('carries the encrypted payload into the backup representation', () => {
  ...
  it('exposes the shared vault uuid and last editor of a shared vault item', () => {
    ...
    expect(projection.last_edited_by).toEqual(lastEditedBy)
  })
  ...
  it('refuses to map a backup representation back to the domain', () => {
