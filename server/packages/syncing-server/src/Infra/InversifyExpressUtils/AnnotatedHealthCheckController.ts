...
export class AnnotatedHealthCheckController {
  constructor(
    @inject(TYPES.Sync_ORMItemRepository) private itemRepository: Repository<SQLItem>,
    ...
    @inject(TYPES.Sync_Redis) @optional() private redis: { ping(): Promise<string> } | undefined,
  ) {}
  ...
  public async readiness(@response() res: Response): Promise<void> {
    ...
    try {
      await this.withTimeout(this.itemRepository.manager.query('SELECT 1'), 2000)
      checks.db = true
    } catch {
      checks.db = false
    }
