...
describe('GRPCServiceProxy', () => {
  ...
  describe('downstream error handling', () => {
    ...
    it('gives up after 3 retries and reports the timeout to the client', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('timed out after 3 retries'), expect.anything())
    ...
    it('logs the failing target URL together with the acting user', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('http://auth/sessions'), { userId: 'u-1' })
    })
