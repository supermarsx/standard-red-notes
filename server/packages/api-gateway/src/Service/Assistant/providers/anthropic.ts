...
import { Provider, ProviderRequest, ProviderEvent, AssistantToolCall, ProviderStopReason } from './types'
      ...
      switch (event.type) {
        ...
        case 'content_block_delta': {
          if (event.delta.type === 'text_delta') {
            ...
            if (pending) {
              pending.argBuf += event.delta.partial_json
            }
          }
          break
        }
...
function mapStop(reason: string | null): ProviderStopReason {
  switch (reason) {
    case 'end_turn':
      return 'end_turn'
    case 'max_tokens':
