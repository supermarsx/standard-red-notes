...
export function listMarkdownFiles(root = repositoryRoot) {
  const result = spawnSync(
    'git',
    ['ls-files', '-z', '--cached', '--others', '--exclude-standard', '--', '*.md', '*.markdown'],
    {
      cwd: root,
      encoding: 'utf8',
      windowsHide: true,
  ...
  if (result.status !== 0) {
    throw new Error(`git ls-files failed: ${result.stderr || `exit code ${result.status}`}`)
  }
...
export async function collectMermaidDocuments(root = repositoryRoot, files = listMarkdownFiles(root)) {
  const documents = []
...
export async function assertMermaidRuntimeContract(root = repositoryRoot) {
  const include = await readFile(path.join(root, 'docs/_includes/mermaid.html'), 'utf8')
  const runtime = await readFile(path.join(root, 'docs/assets/mermaid-init.js'), 'utf8')
  const stylesheetPath = path.join(root, 'docs/assets/mermaid.css')

  if (!include.includes('/assets/mermaid-init.js')) {
    throw new Error('docs/_includes/mermaid.html must load /assets/mermaid-init.js')
  }
  if (!runtime.includes(`mermaid@${MERMAID_VERSION}/`)) {
    throw new Error(`docs/assets/mermaid-init.js must pin Mermaid ${MERMAID_VERSION}`)
  }
  if (!runtime.includes("'strict'")) {
    throw new Error("docs/assets/mermaid-init.js must use Mermaid securityLevel 'strict'")
  }
...
function npxInvocation() {
  ...
  const result = spawnSync('where.exe', ['npx.cmd'], { encoding: 'utf8', windowsHide: true })
  const launcher = result.stdout?.split(/\r?\n/).find(Boolean)
  const cli = launcher && path.join(path.dirname(launcher), 'node_modules/npm/bin/npx-cli.js')
  if (!cli || !existsSync(cli)) {
    throw new Error('Could not resolve the npm npx-cli.js launcher from npx.cmd')
  }
...
export async function renderDiagramWithCli(diagram, index, temporaryDirectory, root = repositoryRoot) {
  ...
  try {
    ...
    throw new Error(`${diagram.file}:${diagram.line}: Mermaid ${MERMAID_VERSION} render failed\n${error.message}`)
  }
}
...
export async function validateMermaidDocuments({
  ...
  const diagrams = await collectMermaidDocuments(root, files ?? listMarkdownFiles(root))
  if (diagrams.length === 0) {
    throw new Error('No Mermaid fences found; the validation gate would not exercise the renderer')
  }
  ...
  console.log(`Rendered ${diagrams.length} Mermaid diagram(s) with Mermaid CLI ${MERMAID_VERSION}.`)
  return diagrams
}
