...
test('extractMermaidFences ignores other code blocks', () => {
  assert.deepEqual(extractMermaidFences('```js\nconst mermaid = true\n```'), [])
})
...
test('extractMermaidFences rejects empty and unterminated diagrams', () => {
  assert.throws(() => extractMermaidFences('```mermaid\n```', 'empty.md'), /empty\.md:1: empty Mermaid fence/)
  assert.throws(
...
test('collectMermaidDocuments requires the Jekyll bootstrap in docs pages', async () => {
  ...
  try {
    ...
    await writeFile(path.join(root, 'docs/diagram.md'), '```mermaid\nflowchart LR\nA --> B\n```\n')

    await assert.rejects(
      collectMermaidDocuments(root, ['docs/diagram.md']),
      new RegExp(`Mermaid fences require ${MERMAID_INCLUDE.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`),
    )
  } finally {
...
test('validateMermaidDocuments invokes the renderer for every diagram', async () => {
  ...
  try {
    ...
    await writeFile(path.join(root, 'docs/_includes/mermaid.html'), '/assets/mermaid-init.js')
    await writeFile(
      path.join(root, 'docs/assets/mermaid-init.js'),
      `const url = 'https://cdn.example/mermaid@${MERMAID_VERSION}/dist/mermaid.esm.min.mjs'; const securityLevel = 'strict'`,
    )
    await writeFile(path.join(root, 'docs/assets/mermaid.css'), '.docs-mermaid {}')
    await writeFile(
