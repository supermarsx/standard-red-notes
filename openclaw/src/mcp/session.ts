...
export class McpSession {
  ...
  async start(): Promise<void> {
    ...
    this.client = new Client({ name: 'openclaw', version: '0.1.0' }, { capabilities: {} })
  ...
  tools(): CatalogEntry[] {
    return this.catalog.filter((t) => this.opts.allowedScopes.includes(t.scope))
  }
  ...
  async call(name: string, args: unknown): Promise<unknown> {
    ...
    if (!this.opts.allowedScopes.includes(entry.scope)) {
      throw new Error(`tool ${name} requires scope ${entry.scope} which is not granted`)
    }
