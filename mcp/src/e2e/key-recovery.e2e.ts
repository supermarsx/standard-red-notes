...
async function main(): Promise<void> {
  ...
  const p1 = [];
  for (let i = 0; i < 3; i++) {
    const n = await clientA.createNote({
      title: `KR-${i}`,
      body: `kr-body-${i}`,
      tags: [],
    });
  ...
  const B = await bootstrapHeadlessApp({
    serverUrl: SERVER,
    dataDir: dirB,
    password: A.password,
    syncIntervalMs: 0,
  ...
  check(
    ...
    p1.every(
      (n) =>
        bNotes.find((x: any) => x.uuid === n.uuid)?.text === p1Bodies[n.uuid],
    ),
  );
  check(
    "no note left errorDecrypting after fresh-device recovery",
    (B.app.items.invalidItems ?? []).length === 0,
  );
  check(
    "the account items key is present on the fresh device",
    B.app.items.getDisplayableItemsKeys().length >= 1,
  );
  ...
  const clientC = new SnjsBackedClient(C.app, {
    allowWrites: true,
    baseUrl: SERVER,
  });
  const P1 = C.password;
  ...
  const oldNote = await clientC.createNote({
    title: "Old",
    body: "old-note-under-K1",
    tags: [],
  });
  await C.app.sync();
  ...
  const D1 = await bootstrapHeadlessApp({
    serverUrl: SERVER,
    dataDir: dirD,
    password: P1,
    syncIntervalMs: 0,
  ...
  await D1.sync();
  await sleep(200);
  const dOld = D1.app.items
    .getDisplayableNotes()
    .find((n: any) => n.uuid === oldNote.uuid);
  check(
    "device D decrypted the old note under K1 (baseline)",
    dOld?.text === "old-note-under-K1",
  );
  await D1.deinit();
  ...
  check(
    "password change (items-key rotation) succeeded on device C",
    !changeResp?.error,
  );
  const newNote = await clientC.createNote({
    title: "New",
    body: "new-note-under-K2",
    tags: [],
  });
  await C.app.sync();
  ...
  const E = await bootstrapHeadlessApp({
    serverUrl: SERVER,
    dataDir: dirE,
    password: P2,
    syncIntervalMs: 0,
  ...
  check(
    "convergence: recovered the PRE-rotation note (old items key K1)",
    eOld?.text === "old-note-under-K1",
  );
  check(
    "convergence: recovered the POST-rotation note (new items key K2)",
    eNew?.text === "new-note-under-K2",
  );
  check(
    "convergence: BOTH items keys recovered (>=2 items keys present)",
    E.app.items.getDisplayableItemsKeys().length >= 2,
  );
  check(
    "convergence: nothing left errorDecrypting after rotation recovery",
    (E.app.items.invalidItems ?? []).length === 0,
  );
  await cleanup(E, dirE);

  // ---- 2b. NO-CLOBBER: stale device D re-opens; good K1/old note must survive ----
  const D2 = await bootstrapHeadlessApp({
    serverUrl: SERVER,
    dataDir: dirD,
    password: P2,
    syncIntervalMs: 0,
  ...
  for (let i = 0; i < 8 && !converged; i++) {
    await D2.sync().catch(() => {});
    await sleep(1000);
    const nn = D2.app.items
      .getDisplayableNotes()
      .find((n: any) => n.uuid === newNote.uuid);
  ...
  console.log(
    ...
    JSON.stringify({
      ...
      stillSignedIn,
      invalidCount,
      syncFailures: health.consecutiveFailures,
    }),
  );
  ...
  if (!stillSignedIn) {
    ...
    check(
      "no-clobber: stale device keeps the old note readable (good K1 not clobbered)",
      dOldAfter?.text === "old-note-under-K1",
    );
    check(
      "no prompt loop / bridge not wedged on the stale device",
      health.consecutiveFailures <= 8,
    );
    if (converged) {
      const dNewAfter = D2.app.items
        .getDisplayableNotes()
        .find((n: any) => n.uuid === newNote.uuid);
      check(
        "stale device converged to the new note in place after recovery",
        dNewAfter?.text === "new-note-under-K2",
      );
    }
  }
