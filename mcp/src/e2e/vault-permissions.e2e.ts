...
async function main(): Promise<void> {
  ...
  const inviteRO = await appA.vaultInvites.inviteContactToSharedVault(
    shared,
    trustedB,
    "read",
  );
  ...
  check(
    "B is classified as a read-only vault member",
    appB.vaultUsers.isCurrentUserReadonlyVaultMember(findBySharedUuid(appB)) ===
      true,
  );
  const membersRO = await appA.vaultUsers.getSharedVaultUsersFromServer(shared);
  const bUser = (membersRO ?? []).find(
    (u: { user_uuid: string }) =>
  ...
  check(
    "owner does NOT see the read-only member's rejected write",
    ownerBodyUnchanged,
  );
  ...
  await appA.vaultUsers.removeUserFromSharedVault(
    shared,
  ...
  const inviteRW = await appA.vaultInvites.inviteContactToSharedVault(
    sharedRefetch,
    trustedB,
    "write",
  );
