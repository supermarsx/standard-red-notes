...
async function main(): Promise<void> {
  ...
  const created = await client.createNote({ title: 'Revision Note', body: 'v1', tags: [] })
  await A.app.sync()
  ...
  try {
    const sres = await fetch(`${SERVER}/v1/sessions`, { headers: { authorization: `Bearer ${token}` } })
    const sbody: any = await sres.json().catch(() => ({}))
    const roles = (sbody?.meta?.auth?.roles ?? []).map((r: any) => r?.name)
    console.log('    account roles:', JSON.stringify(roles), '=> isFreeUser:', roles.length === 1 && roles[0] === 'CORE_USER')
  } catch {
    /* diagnostic only */
  }
  ...
  const rawRes = await fetch(revUrl, { headers: { authorization: `Bearer ${token}` } })
  const rawBody = (await rawRes.text()).slice(0, 120)
  console.log('    RAW GET /v2/items/:uuid/revisions ->', rawRes.status, JSON.stringify(rawBody))
  ...
  console.log('    snjs listRevisions ok?', listOk, 'err:', listOk ? '' : String(listRes?.getError?.() ?? ''))

  // EVIDENCE of the reported product bug: the revisions endpoint must be reachable.
  check('PRODUCT BUG (report): GET /v2/items/:uuid/revisions is reachable (not 404)', rawRes.status !== 404)
  check('PRODUCT BUG (report): snjs listRevisions succeeds (history retrievable)', listOk)
  ...
  check('getRevision on a non-existent revision fails cleanly (no crash)', bogus?.isFailed?.() === true)
  ...
  if (revisions.length === 0) {
    skip('revision retrieve/restore/delete', 'endpoint reachable but no revision accrued within the window (300s premium gate)')
    await cleanup(A.app, A.dataDir)
    finish()
    return
  }
  ...
  const getRes = await appA.getRevision.execute({ itemUuid: created.uuid, revisionUuid: oldest.uuid })
  check('an older revision is retrievable and decrypts', !getRes.isFailed())
  if (!getRes.isFailed()) {
    const oldText = getRes.getValue()?.payload?.content?.text
    const liveNote = appA.items.getDisplayableNotes().find((n: any) => n.uuid === created.uuid)
    await appA.mutator.changeItem(liveNote, (m: any) => {
      m.text = oldText
    })
    await A.app.sync()
    const restored = appA.items.getDisplayableNotes().find((n: any) => n.uuid === created.uuid)
    check('note reverted to the restored revision body', restored?.text === oldText)
    const delRes = await appA.deleteRevision.execute({ itemUuid: created.uuid, revisionUuid: oldest.uuid })
