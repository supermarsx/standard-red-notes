...
async function main(): Promise<void> {
  ...
  const itemsKeys = A.app.items.getDisplayableItemsKeys().map((k: any) => ({
    uuid: k.uuid,
    itemsKey: k.itemsKey,
  ...
  const serverToken: string | undefined =
    createBody?.data?.token ?? createBody?.token;
  check(
    "POST /v1/mcp-tokens returns a server token",
  ...
  check(
    "authenticating with the fresh token yields a session",
    hasSession(auth1),
  );
  ...
  const list =
    (listBody?.data ?? listBody)?.mcpTokens ??
    (listBody?.data ?? listBody)?.tokens ??
    [];
  const listed = Array.isArray(list)
    ? list.find((t: any) => t.uuid === tokenUuid)
    : undefined;
  ...
  check(
    "authenticating with a revoked token fails (no session)",
    !hasSession(auth2),
  );
