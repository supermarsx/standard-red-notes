...
async function supportsLocalOnly(appA: any): Promise<boolean> {
  const probe = await appA.mutator.createItem(
    ContentType.TYPES.Note,
    { title: "__probe", text: "x", references: [] },
    false,
  );
  await appA.mutator.changeItem(
    appA.items
      .getDisplayableNotes()
      .find((n: { uuid: string }) => n.uuid === probe.uuid),
    (m: any) => {
      m.localOnly = true;
    },
  );
  const read = appA.items
    .getDisplayableNotes()
    .find((n: { uuid: string }) => n.uuid === probe.uuid);
...
async function main(): Promise<void> {
  ...
  await appA.mutator.changeItem(
    appA.items
      .getDisplayableNotes()
      .find((n: { uuid: string }) => n.uuid === noteUuid),
    (m: { localOnly: boolean }) => {
      m.localOnly = true;
    },
  );
  ...
  const localBeforeReload = appA.items
    .getDisplayableNotes()
    .find((n: { uuid: string }) => n.uuid === noteUuid);
  check("note exists locally after edits+syncs", !!localBeforeReload);
  check(
    "local note carries the latest edit (v3) before reload",
    localBeforeReload?.text === "v3",
  );
  check(
    "local note is flagged localOnly before reload",
    localBeforeReload?.localOnly === true,
  );
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (
      app2.app.items
        .getDisplayableNotes()
        .some((n: { uuid: string }) => n.uuid === noteUuid)
    ) {
      leaked = true;
      break;
    }
  ...
  check(
    "local-only note NEVER reaches a second device (server never got it)",
    !leaked,
  );
  ...
  await A.app.deinit();
  const reloaded = await bootstrapHeadlessApp({
    serverUrl: SERVER,
    dataDir,
    password: A.password,
    syncIntervalMs: 0,
  });
  const reloadedNote = reloaded.app.items
    .getDisplayableNotes()
    .find((n: { uuid: string }) => n.uuid === noteUuid);
  check(
    "RELOAD: local-only note survives a deinit + fresh re-bootstrap from disk",
    !!reloadedNote,
  );
  check(
    "RELOAD: the note still carries its latest edit (v3)",
    reloadedNote?.text === "v3",
  );
  check(
    "RELOAD: the note is still flagged localOnly",
    reloadedNote?.localOnly === true,
  );
