...
async function main(): Promise<void> {
  ...
  const normal = await createShare(token, {
    type: "note",
    encryptedPayload: payload,
  });
  check(
    "POST /v1/shares creates a share (200 + shareId)",
    normal.status === 200 && Boolean(normal.shareId),
  );
  if (normal.shareId) {
    const read1 = await publicGet(normal.shareId);
    const got = read1.data?.data ?? read1.data;
    check(
      "public GET /v1/shares/:id returns the ciphertext payload",
      read1.status === 200 && got?.encryptedPayload === payload,
    );
    check(
      "public share read does not leak the owner uuid",
      !leaksOwnerUuid(read1.data),
    );
    ...
    check(
      "DELETE /v1/shares revokes the share (200)",
      revokeRes.status === 200,
    );
    const afterRevoke = await publicGet(normal.shareId);
    check(
      "a revoked share returns 404 on public read",
      afterRevoke.status === 404,
    );
    check(
      "the 404 for a revoked share does not leak the owner uuid",
      !leaksOwnerUuid(afterRevoke.data),
    );
  }

  // 3. A one-time-view ("burn") share is readable exactly once, then consumed.
  const burn = await createShare(token, {
    type: "note",
    encryptedPayload: payload,
    oneTimeView: true,
  });
  check(
    "POST /v1/shares creates a one-time-view share",
    burn.status === 200 && Boolean(burn.shareId),
  );
  if (burn.shareId) {
    ...
    if (got?.oneTimeView !== true) {
      ...
      check(
        "the first read reports the share as one-time-view",
        got?.oneTimeView === true,
      );
      const secondOpen = await publicGet(burn.shareId);
      check(
        "the second public read of a one-time-view share returns 404 (burned)",
        secondOpen.status === 404,
      );
    }
  }
