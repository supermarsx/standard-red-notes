...
async function main(): Promise<void> {
  ...
  const noteA = await clientA.createNote({
    title: "A",
    body: "online base",
    tags: [],
  });
  ...
  const app2 = await bootstrapHeadlessApp({
    serverUrl: SERVER,
    dataDir: dir2,
    password: A.password,
    syncIntervalMs: 0,
  ...
  await app2.sync();
  const client2 = new SnjsBackedClient(app2, {
    allowWrites: true,
    baseUrl: SERVER,
  });
  check(
    "device 2 pulled the base note",
    (await client2.listNotes(100)).notes.some((n) => n.uuid === noteA.uuid),
  );
  ...
  const offlineNote = await appA.mutator.createItem(
    ...
    {
      title: "B",
      body: "authored offline",
      text: "authored offline",
      references: [],
    },
    true,
  );
  const aNoteA = appA.items
    .getDisplayableNotes()
    .find((n: { uuid: string }) => n.uuid === noteA.uuid);
  ...
  const offlineLocal = appA.items
    .getDisplayableNotes()
    .find((n: { uuid: string }) => n.uuid === offlineNote.uuid);
  check(
    "offline-authored note exists locally before reconnect",
    !!offlineLocal,
  );
  const dirtyItems = appA.items.getDirtyItems?.() ?? [];
  check(
    "offline changes are queued (dirty) pending upload",
    dirtyItems.some((i: { uuid: string }) => i.uuid === offlineNote.uuid),
  );

  // Device 2 must NOT have the offline note yet (device 1 hasn't synced).
  await app2.sync();
  const leakedEarly = (await client2.listNotes(200)).notes.some(
    (n) => n.uuid === offlineNote.uuid,
  );
  check(
    "offline note has NOT reached device 2 before device 1 reconnects",
    !leakedEarly,
  );
  ...
  const sawNew = await (async () => {
    for (let i = 0; i < 12; i++) {
      await app2.sync();
      if (
        (await client2.listNotes(200)).notes.some(
          (n) => n.uuid === offlineNote.uuid,
        )
      )
        return true;
  ...
  check(
    "after reconnect, device 2 receives the offline EDIT to the base note",
    sawEdit,
  );

  // === Offline DELETE then reconnect ===
  const aNoteAForDelete = appA.items
    .getDisplayableNotes()
    .find((n: { uuid: string }) => n.uuid === noteA.uuid);
  ...
  const sawDelete = await (async () => {
    for (let i = 0; i < 12; i++) {
      await app2.sync();
      if (
        !(await client2.listNotes(200)).notes.some((n) => n.uuid === noteA.uuid)
      )
        return true;
      await sleep(800);
    }
  ...
  check(
    "after reconnect, device 2 sees the offline DELETE of the base note",
    sawDelete,
  );
