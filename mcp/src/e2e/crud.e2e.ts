...
async function main() {
  ...
  check(
    "readNote returns title/body/tags",
    read.title === "Shopping" &&
      read.body === "milk, eggs" &&
      read.tags.includes("errands"),
  );

  const search = await client.searchNotes("eggs", 5);
  check(
    "searchNotes finds by body",
    search.hits.some((h) => h.uuid === a.uuid),
  );

  await client.updateNote(a.uuid, {
    body: "milk, eggs, bread",
    tags: ["urgent"],
  });
  const reread = await client.readNote(a.uuid);
  check(
    "updateNote persists body + tag",
    reread.body === "milk, eggs, bread" && reread.tags.includes("urgent"),
  );

  const tags = await client.listTags();
  check(
    "listTags returns created tags",
    tags.some((t) => t.title === "errands") &&
      tags.some((t) => t.title === "work"),
  );

  const vault = await client.createVault("Team", "shared");
  check("createVault returns a vault", !!vault.uuid);
  const vnote = await client.createNote({
    title: "In vault",
    body: "x",
    tags: ["shared"],
    vault: vault.uuid,
  ...
  check(
    ...
    !afterDelete.notes.some((n) => n.uuid === b.uuid),
  );
