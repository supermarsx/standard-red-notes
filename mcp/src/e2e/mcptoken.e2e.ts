...
async function main(): Promise<void> {
  ...
  const createdNote = await ca.createNote({
    title: "MCP token note",
    body: known,
    tags: [],
  });
  ...
  const accessToken: string | undefined =
    A.app.sessions.getSession()?.accessToken?.value ??
    A.app.sessions.getSession()?.accessToken;
  ...
  check(
    "full token has 3 dot-separated parts",
    fullToken.split(".").length === 3,
  );
  ...
  try {
    ...
    check(
      "bridge B authenticated with token (session layer signed in)",
      B.app.sessions.isSignedIn() === true,
    );
    check("bridge B scope is read-only", result.readOnly === true);
    check(
      "bridge B injected at least one items key",
      B.app.items.getDisplayableItemsKeys().length > 0,
    );

    // Sync once more and look for A's note, fully decrypted.
    await B.sync().catch(() => {});
    const note = B.app.items
      .getDisplayableNotes()
      .find((n: any) => n.uuid === createdNote.uuid);
    check("bridge B can see A note in its store", Boolean(note));
    bDecrypted = Boolean(note) && note.text === known;
    check(
      "bridge B DECRYPTED A note content (cross-impl crypto + injection)",
      bDecrypted,
    );
    ...
    if (targetNote) {
      ...
      await A.sync().catch(() => {});
      const aNote = A.app.items
        .getDisplayableNotes()
        .find((n: any) => n.uuid === createdNote.uuid);
      const persisted = aNote?.text === editMarker;
      check(
        "read-only session did NOT persist a direct write to the server",
        persisted === false,
      );
    } else {
      check(
