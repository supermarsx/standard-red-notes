...
async function keyParamsWithAppPassword(
  ...
  try {
    ...
    const satisfied =
      res.status === 200 && typeof params?.pw_nonce === "string";
...
async function main(): Promise<void> {
  ...
  check(
    "POST /v1/app-passwords returns the app password uuid",
    Boolean(appPasswordUuid),
  );
  ...
  const listedEntry = Array.isArray(list)
    ? list.find((e: any) => e.uuid === appPasswordUuid)
    : undefined;
  check(
    "GET /v1/app-passwords lists the created app password",
    Boolean(listedEntry),
  );
  check(
  ...
  check(
    "TOTP 2FA enabled on the account (gate for the app password)",
    mfaActive === true,
  );
  ...
  check(
    "with 2FA on, key params without a second factor are gated",
    noFactor.satisfied === false,
  );
  ...
  check(
    "a valid app password satisfies the 2FA gate (real key params returned)",
    withAppPw.satisfied === true,
  );

  // A wrong app password does NOT satisfy the gate (fail-closed).
  const wrongAppPw = await keyParamsWithAppPassword(
    email,
    appPasswordSecret + "x",
  );
  check(
    "a wrong app password does not satisfy the 2FA gate",
    wrongAppPw.satisfied === false,
  );
  ...
  check(
    "DELETE /v1/app-passwords revokes the app password",
    delRes.status === 200,
  );

  const afterRevoke = await keyParamsWithAppPassword(email, appPasswordSecret);
  check(
    "a revoked app password no longer satisfies the 2FA gate",
    afterRevoke.satisfied === false,
  );
  ...
  check(
    ...
    Array.isArray(remaining) &&
      !remaining.some((e: any) => e.uuid === appPasswordUuid),
  );
