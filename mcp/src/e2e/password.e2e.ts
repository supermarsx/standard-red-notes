...
async function trySignIn(
  ...
  ok: boolean;
  app?: Awaited<ReturnType<typeof bootstrapHeadlessApp>>;
  dataDir: string;
}> {
...
async function main(): Promise<void> {
  ...
  const note = await client.createNote({
    title: "Pre-change note",
    body: "encrypted under the old key",
    tags: ["secret"],
  });
  await app.sync();
  ...
  const result = await app.app.changePassword(
    password,
    newPassword,
    undefined,
    undefined,
  ...
  if (withNew.ok && withNew.app) {
    ...
    const c2 = new SnjsBackedClient(withNew.app, {
      allowWrites: true,
      baseUrl: SERVER,
    });
    ...
    if (found) {
      ...
      check(
        "the pre-change note still decrypts to its original body",
        read.body === "encrypted under the old key" &&
          read.tags.includes("secret"),
      );
    }
