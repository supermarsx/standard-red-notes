...
async function main(): Promise<void> {
  ...
  check(
    "fresh account has a session access token",
    typeof token === "string" && token!.length > 0,
  );
  check(
    "fresh account has a user uuid",
  ...
  check(
    "a subscription object is present for an unsubscribed account",
    !!subscription,
  );
  check(
    "included subscription is the PRO plan (no free-tier gating)",
    subscription?.planName === PRO_PLAN,
  );
  check(
    "included subscription is not cancelled",
    subscription?.cancelled === false,
  );
  check(
    "included subscription does not expire in the near future (far-future endsAt)",
    typeof subscription?.endsAt === "number" &&
      subscription.endsAt > Date.now() * 1000,
  );
  check(
    "subscription response echoes the user identity",
    sub.body?.user?.uuid === uuid,
  );
  ...
  const features: any[] = Array.isArray(feat.body?.features)
    ? feat.body.features
    : [];
  check(
    "features list is non-empty (full set provisioned)",
    features.length > 0,
  );
  check(
    "every feature is granted at the PRO_USER role",
    features.every((f) => f?.role_name === PRO_USER),
  );
  check(
  ...
  const identifiers = features
    .map((f) => f?.identifier)
    .filter((id): id is string => typeof id === "string");
  check(
    "features expose stable identifiers",
    identifiers.length === features.length,
  );
  ...
  const isUnknownSetting = (r: { status: number; body: any }): boolean =>
    r.status === 400 &&
    /invalid setting name/i.test(String(r.body?.error?.message ?? ""));
  ...
  const setBackup = await putSetting(
    uuid,
    token,
  ...
  check(
    "setting EMAIL_BACKUP_FREQUENCY succeeds",
    setBackup.status === 200 && setBackup.body?.success === true,
  );
  const readBackup = await getJson(
    `${SERVER}/v1/users/${uuid}/settings/EMAIL_BACKUP_FREQUENCY`,
    token,
  );
  check(
  ...
  const setMute = await putSetting(
    uuid,
    token,
    "MUTE_MARKETING_EMAILS",
    "muted",
  );
  check(
    "setting MUTE_MARKETING_EMAILS succeeds",
    setMute.status === 200 && setMute.body?.success === true,
  );
  check(
    "an unsensitive setting is flagged sensitive:false",
    setMute.body?.setting?.sensitive === false,
  );
  const readMute = await getJson(
    `${SERVER}/v1/users/${uuid}/settings/MUTE_MARKETING_EMAILS`,
    token,
  );
  check(
    "MUTE_MARKETING_EMAILS reads back its value",
    readMute.body?.setting?.value === "muted",
  );
  ...
  const setReminder = await putSetting(
    uuid,
    token,
  ...
  if (isUnknownSetting(setReminder)) {
    skip(
      "EMAIL_REMINDERS_ENABLED round-trip",
      "setting name not in deployed build (fork-added)",
    );
  } else {
    check(
      "setting EMAIL_REMINDERS_ENABLED succeeds",
      setReminder.status === 200 && setReminder.body?.success === true,
    );
    const readReminder = await getJson(
      `${SERVER}/v1/users/${uuid}/settings/EMAIL_REMINDERS_ENABLED`,
      token,
    );
    check(
  ...
  check(
    "setting EXTENSION_KEY (sensitive) succeeds",
    setExt.status === 200 && setExt.body?.success === true,
  );
  ...
  check(
    ...
    setExt.body?.setting === undefined ||
      setExt.body?.setting?.value === undefined,
  );
  const readExt = await getJson(
    `${SERVER}/v1/users/${uuid}/settings/EXTENSION_KEY`,
    token,
  );
  check("reading a sensitive setting returns 200", readExt.status === 200);
  check(
    "a sensitive setting is NOT returned as a value (existence only)",
    readExt.body?.setting === undefined,
  );
  check(
    "a sensitive setting value is never echoed in the response body",
    !JSON.stringify(readExt.raw).includes(EXT_SECRET),
  );

  // The bulk settings listing must likewise never leak the sensitive value.
  const allSettings = await getJson(
    `${SERVER}/v1/users/${uuid}/settings`,
    token,
  );
  check("settings listing returns 200", allSettings.status === 200);
  check(
    "settings listing does not leak the sensitive value",
    !JSON.stringify(allSettings.raw).includes(EXT_SECRET),
  );
  ...
  if (isUnknownSetting(setNc)) {
    skip(
      "NEXTCLOUD_BACKUP_APP_PASSWORD sensitivity",
      "setting name not in deployed build (fork-added)",
    );
  } else {
    check(
      "setting NEXTCLOUD_BACKUP_APP_PASSWORD succeeds",
      setNc.status === 200 && setNc.body?.success === true,
    );
    const readNc = await getJson(
      `${SERVER}/v1/users/${uuid}/settings/NEXTCLOUD_BACKUP_APP_PASSWORD`,
      token,
    );
    check(
      "the Nextcloud app password is NOT returned as a value",
      readNc.body?.setting === undefined,
    );
    check(
      "the Nextcloud app password is never echoed in the response body",
      !JSON.stringify(readNc.raw).includes(NC_SECRET),
    );
  }
  ...
  const adminLookup = await fetch(
    `${SERVER}/v1/admin/lookup-user/someone%40example.com`,
    {
      headers: { authorization: `Bearer ${token}` },
    },
  );
  ...
  const foreignSub = await getJson(
    `${SERVER}/v1/users/${otherUuid}/subscription`,
    token,
  );
  check(
