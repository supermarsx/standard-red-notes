...
const { UuidGenerator } = snjs as unknown as {
...
async function main(): Promise<void> {
  ...
  const original = new TextEncoder().encode(
    "encrypted attachment payload — " + "lorem ipsum ".repeat(40),
  );
  const uuid = UuidGenerator.GenerateUuid();

  const operation = await filesA.beginNewFileUpload(original.length);
  check(
    "beginNewFileUpload returned an upload operation",
    operation && typeof operation.getProgress === "function",
  );
  ...
  for (let offset = 0; offset < original.length; offset += chunkSize) {
    const chunk = original.subarray(
      offset,
      Math.min(offset + chunkSize, original.length),
    );
    const isLast = offset + chunkSize >= original.length;
    const err = await filesA.pushBytesForUpload(
      operation,
      chunk,
      index++,
      isLast,
  ...
  const fileItem = await filesA.finishUpload(
    operation,
    { name: "attachment.txt", mimeType: "text/plain" },
    uuid,
  );
  const uploadOk =
    fileItem && typeof fileItem === "object" && "uuid" in fileItem;
  ...
  if (file2) {
    const dlErr = await app2.app.files.downloadFile(
      file2,
  ...
  check(
    "the downloaded+decrypted bytes match the original exactly",
    received.length === original.length &&
      got === new TextDecoder().decode(original),
  );
