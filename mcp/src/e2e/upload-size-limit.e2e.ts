...
function configuredCap(): number | undefined {
  const raw =
    process.env.MAX_ATTACHMENT_BYTE_SIZE_E2E ??
    process.env.MAX_ATTACHMENT_BYTE_SIZE;
...
async function main(): Promise<void> {
  ...
  if (pushError) {
    ...
    try {
      const fileItem = await filesA.finishUpload(
        operation,
        { name: "too-big.bin", mimeType: "application/octet-stream" },
        uuid,
      );
      ...
      if (ok)
        console.log(
          "  unexpected: finishUpload SUCCEEDED for an over-cap file:",
          (fileItem as any).uuid,
        );
    } catch {
      rejected = true;
    }
  ...
  check(
    "an upload exceeding MAX_ATTACHMENT_BYTE_SIZE is rejected at session finish",
    rejected,
  );
