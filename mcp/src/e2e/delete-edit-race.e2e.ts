...
async function main(): Promise<void> {
  ...
  await appB.sync();
  const bBase = appB.app.items
    .getDisplayableNotes()
    .find((n: { uuid: string }) => n.uuid === created.uuid);
  ...
  const liveA = A.app.app.items
    .getDisplayableNotes()
    .filter(
      (n: { uuid: string; conflictOf?: string }) =>
        n.uuid === created.uuid || n.conflictOf === created.uuid,
    )
    .map((n: { uuid: string; text?: string }) => ({
      uuid: n.uuid,
      text: n.text ?? "",
    }))
  ...
  check(
    "devices A and B CONVERGE to the same final set (no split-brain)",
    JSON.stringify(liveA) === JSON.stringify(liveB),
  );

  const editSurvived = liveA.some(
    (n: { text: string }) => n.text === "edited by B after A deleted",
  );
  const deleteWon = liveA.length === 0;
  check(
    "outcome is deterministic (either delete wins OR edit resurrected — not a partial/torn state)",
    deleteWon || editSurvived,
  );
  if (deleteWon) {
    console.log(
      "  info - VERDICT: delete-wins — B's edit did NOT resurrect (last-writer=delete). No split-brain.",
    );
  } else if (editSurvived) {
    console.log(
      "  info - VERDICT: edit-resurrected — B's edit survived as a live note (no silent loss). No split-brain.",
    );
  }
