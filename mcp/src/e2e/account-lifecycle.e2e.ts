...
async function keyParamsProbe(
  ...
  const res = await fetch(`${SERVER.replace(/\/$/, "")}/v2/login-params`, {
    ...
    body: JSON.stringify({
      api: "20240226",
      email,
      code_challenge: "srn-e2e-probe-challenge",
    }),
  });
...
async function rawSignIn(
  ...
  const { SNWebCrypto } =
    (await import("@standardnotes/sncrypto-web")) as unknown as {
      SNWebCrypto: new () => any;
    };
  ...
  try {
    ...
    const codeChallenge = crypto.base64URLEncode(
      await crypto.sha256(codeVerifier),
    );
 ...
 */
async function revokeSessionRaw(
  accessTok: string,
  sessionUuid: string,
): Promise<number> {
  const res = await fetch(
    `${SERVER.replace(/\/$/, "")}/v1/sessions/${sessionUuid}`,
    {
      method: "DELETE",
      headers: { authorization: `Bearer ${accessTok}` },
    },
...
async function main(): Promise<void> {
  ...
  const dupApp = await bootstrapHeadlessApp({
    serverUrl: SERVER,
    dataDir: dupDir,
    password,
    syncIntervalMs: 0,
  ...
  check(
    "duplicate-email register is rejected",
    dupRejected && !dupApp.isSignedIn(),
  );
  ...
  check(
    "the signed-in session has an access token",
    good.ok && !!accessToken(good.app!),
  );
  if (good.app) {
  ...
  const GENERIC_AUTH_ERR =
    /invalid email or password|invalid login credentials|invalid password/i;
  ...
  if (!GENERIC_AUTH_ERR.test(wrongPw.message)) {
    console.log(
      "  (wrong-password message was:",
      JSON.stringify(wrongPw.message),
      ")",
    );
  }
  check(
  ...
  const unknown = await rawSignIn(
    unknownEmail,
    "deliberately-wrong-server-password",
  );
  ...
  if (unknown.message !== wrongPw.message) {
    console.log(
      ...
      JSON.stringify(unknown.message),
      "vs wrong-pw:",
      JSON.stringify(wrongPw.message),
      ")",
    );
  ...
  const kpId = (b: any): string | undefined =>
    b?.data?.identifier ?? b?.identifier;
  const kpNonce = (b: any): string | undefined =>
    b?.data?.pw_nonce ?? b?.pw_nonce;
  const kpErrTag = (b: any): string | undefined =>
    b?.data?.error?.tag ?? b?.error?.tag;
  ...
  const isPseudoChallenge = (r: { status: number; body: any }): boolean =>
    r.status === 401 &&
    /mfa-required|u2f-required/i.test(kpErrTag(r.body) ?? "");
  ...
  if (!isPseudoParams(unknownKp) && !isPseudoChallenge(unknownKp)) {
    console.log(
      "  (unknown key-params: status",
      unknownKp.status,
      "body",
      JSON.stringify(unknownKp.body).slice(0, 240),
      ")",
    );
  }
  check(
    "key-params for a KNOWN email returns 200 with identifier + nonce",
    isPseudoParams(knownKp) || isPseudoChallenge(knownKp),
  );
  check(
  ...
  check(
    "the UNKNOWN email key-params never expose a not-found error",
    !/not.?found|does.?not.?exist|no.?(such|user)/i.test(
      JSON.stringify(unknownKp.body),
    ),
  );
  ...
  if (refDev.ok && refDev.app) {
    ...
    const newAccess =
      refreshed.body?.session?.access_token ??
      refreshed.body?.data?.session?.access_token;
    check(
      "session refresh returns a new access token",
      refreshed.status === 200 && !!newAccess,
    );
    check(
      "the refreshed access token differs from the original",
      !!newAccess && newAccess !== at,
    );
    if (newAccess) {
      check(
        "the refreshed access token is accepted by an authed endpoint",
        await tokenStillValid(newAccess),
      );
    }
  ...
  check(
    "two devices signed in for the session-revoke test",
    dev1.ok && dev2.ok,
  );
  if (dev1.ok && dev1.app && dev2.ok && dev2.app) {
    ...
    check(
      "device-2's token works before revocation",
      await tokenStillValid(dev2Token),
    );
    ...
    check(
      "listing sessions returns at least two entries",
      sessions.length >= 2,
    );
    ...
    check(
      "a revoked session's token stops working",
      (await tokenStillValid(dev2Token)) === false,
    );
  ...
  // than fake a pass.
  {
    ...
    if (!at) {
      ...
      const res = await fetch(
        `${SERVER.replace(/\/$/, "")}/v1/recovery/codes`,
        {
          method: "POST",
          headers: {
      ...
      if (res.status === 200 && typeof codes === "string" && codes.length > 0) {
        ...
        console.log(
          "  SKIP recovery-codes: endpoint requires server-password re-auth (status",
          res.status,
          ")",
        );
      } else {
        console.log(
          "  SKIP recovery-codes: endpoint not available as expected (status",
          res.status,
          ")",
        );
      }
  ...
  try {
    const cpResult = await app.app.changePassword(
      password,
      newPassword,
      undefined,
      undefined,
  ...
  check(
    "after change-password, the OLD password is rejected",
    withOld.ok === false,
  );
  ...
  if (withNew.ok && withNew.app) {
    ...
    if (at && userUuid) {
      const res = await fetch(
        `${SERVER.replace(/\/$/, "")}/v1/users/${userUuid}`,
        {
          method: "DELETE",
      ...
      if (res.status === 200 || res.status === 204) {
        ...
        check(
          "after account deletion, sign-in is rejected",
          afterDelete.ok === false,
        );
        if (afterDelete.app)
          await cleanup(afterDelete.app, afterDelete.dataDir);
      } else {
        console.log(
          "  SKIP delete-account: endpoint requires extra re-auth (status",
          res.status,
          ")",
        );
      }
      ...
      console.log(
        "  SKIP delete-account: missing access token or user uuid on the bridge",
      );
    }
