...
async function deriveRegistration(
  ...
  const snjsMod = (await import("@standardnotes/snjs")) as unknown as Record<
    string,
    any
  >;
  const origination =
    snjsMod.KeyParamsOrigination?.Registration ?? "registration";
  const rootKey = await app.encryption.createRootKey(
    email,
    password,
    origination,
  );
...
async function main(): Promise<void> {
  ...
  const flagOn =
    regB.status === 200 &&
    Boolean(regB.data?.session ?? regB.data?.data?.session);

  if (!flagOn) {
    const msg =
      regB.data?.error?.message ??
      regB.data?.data?.error?.message ??
      `HTTP ${regB.status}`;
    console.log(
  ...
  check(
    "registering the same email in a second workspace succeeds (flag ON)",
    flagOn,
  );
  ...
  check(
    ...
    regC.status === 200 &&
      Boolean(regC.data?.session ?? regC.data?.data?.session),
  );
  ...
  const dupMsg =
    dupB.data?.error?.message ?? dupB.data?.data?.error?.message ?? "";
  check(
  ...
  check(
    "sign-in resolves workspace B by (email, workspace_identifier)",
    signInSucceeded(signInB),
  );

  const signInDefault = await rawSignIn(email, reg.serverPassword, undefined);
  check(
    "sign-in resolves the default workspace for the same email",
    signInSucceeded(signInDefault),
  );
  ...
  const uuidDefault = (
    signInDefault.data?.user ?? signInDefault.data?.data?.user
  )?.uuid;
  check(
  ...
  const signInGhost = await rawSignIn(
    email,
    reg.serverPassword,
    "never-registered-" + Date.now(),
  );
  check(
    "sign-in for an unregistered workspace is rejected",
    !signInSucceeded(signInGhost),
  );
