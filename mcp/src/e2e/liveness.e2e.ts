...
async function main(): Promise<void> {
  ...
  const gw = await fetch(`${GATEWAY_HTTP}/health`)
    .then((r) => r.status)
    .catch(() => 0);
  ...
  const app1 = await bootstrapHeadlessApp({
    serverUrl: SERVER,
    dataDir: dir1,
    password,
    syncIntervalMs: 1500,
  ...
  check(
    "session layer is authenticated",
    app1.app.sessions.isSignedIn() === true,
  );
  check(
    "online download-first sync completed (not stuck offline)",
    app1.app.sync.completedOnlineDownloadFirstSync === true,
  );
  ...
  const remote = await c2.createNote({
    title: "From device 2",
    body: "pulled by loop",
    tags: [],
  });
  ...
  while (Date.now() < deadline) {
    ...
    if (
      app1.app.items
        .getDisplayableNotes()
        .some((n: { uuid: string }) => n.uuid === remote.uuid)
    ) {
      pulled = true;
      break;
    }
  ...
  check(
    "background sync loop pulled a remote note with no manual sync",
    pulled,
  );
  ...
  check(
    "session still authenticated after loop cycles",
    app1.app.sessions.isSignedIn() === true,
  );
