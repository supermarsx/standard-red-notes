...
async function main(): Promise<void> {
  ...
  check(
    "both accounts produced a collaboration ID",
    !!collabA && !!collabB && collabA !== collabB,
  );
  ...
  check(
    "A converted the vault into a shared vault",
    !!shared && !!shared.sharing?.sharedVaultUuid,
  );
  await A.app.sync();

  // 3. A invites B (write permission).
  const invitable =
    await appA.vaultInvites.getInvitableContactsForSharedVault(shared);
  check(
    "B is an invitable contact for the shared vault",
    invitable.some(
      (c: { contactUuid: string }) => c.contactUuid === trustedB.contactUuid,
    ),
  );
  const inviteResult = await appA.vaultInvites.inviteContactToSharedVault(
    shared,
    trustedB,
    "write",
  );
  ...
  const record = await pollFor("B inbound invite", async () => {
    ...
    return records.find(
      (r: { invite: { shared_vault_uuid: string }; trusted: boolean }) =>
        r.invite.shared_vault_uuid === shared.sharing.sharedVaultUuid,
    );
  });
  check("B received the inbound invite", !!record);
  check(
    "the invite is trusted (A is a known contact)",
    record?.trusted === true,
  );
  ...
  const note = await clientA.createNote({
    title: "Shared plan",
    body: "agent wrote this in the shared vault",
    tags: ["team"],
    vault: shared.uuid,
  });
  await A.app.sync();
  ...
  if (seenByB) {
    ...
    check(
      "B decrypted the shared note body + vault",
      read.body === "agent wrote this in the shared vault" &&
        read.vault === "Team Space",
    );
  }
  ...
  const editSeenByA = await pollFor("A sees B edit", async () => {
    ...
    return read && read.body === "human edited the shared note"
      ? read
      : undefined;
  });
  check(
    "A sees B's edit to the shared note (bidirectional collaboration)",
    !!editSeenByA,
  );
