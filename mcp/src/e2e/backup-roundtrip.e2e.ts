...
async function main(): Promise<void> {
  ...
  const encBackup = unwrap<any>(
    await appA.createEncryptedBackupFile.execute({ skipAuthorization: true }),
  );
  check(
    "encrypted backup created with items",
    !!encBackup && Array.isArray(encBackup.items) && encBackup.items.length > 0,
  );
  check(
    "encrypted backup carries keyParams (needed to restore)",
    !!encBackup.keyParams,
  );
  const encBlob = JSON.stringify(encBackup.items);
  check(
    "encrypted backup contains NO plaintext note body",
    !encBlob.includes("body-A-secret-alpha"),
  );
  check(
    "encrypted backup INCLUDES the items key(s) (round-trip restorable)",
    encBackup.items.some((i: any) =>
      String(i.content_type).includes("ItemsKey"),
    ),
  );
  ...
  const restore = await bootstrapHeadlessApp({
    serverUrl: SERVER,
    dataDir: dirR,
    password: A.password,
    syncIntervalMs: 0,
  ...
  check(
    "all 3 notes restored from the encrypted backup",
    uuids.every((u) => restored.some((r: any) => r.uuid === u)),
  );
  check(
    "restored notes DECRYPTED back to their original bodies",
    uuids.every(
      (u) => restored.find((r: any) => r.uuid === u)?.text === bodies[u],
    ),
  );
  check(
  ...
  check(
    "found the note item to corrupt",
    !!victim && typeof victim.content === "string",
  );
  ...
  const tampered =
    head +
    c.slice(8).replace(/[A-Za-z]/g, (ch: string) => (ch === "A" ? "B" : "A"));
  ...
  check(
    ...
    afterPerItem.some(
      (r: any) => r.uuid === victimUuid && r.text === bodies[victimUuid],
  ...
  check(
    ...
    uuids
      .slice(1)
      .every(
        (u) => afterPerItem.find((r: any) => r.uuid === u)?.text === bodies[u],
      ),
  );
  ...
  check(
    ...
    uuids
      .slice(1)
      .every(
        (u) => afterStruct.find((r: any) => r.uuid === u)?.text === bodies[u],
      ),
  );
  ...
  if (decResult === TIMEOUT || decResult?.isFailed?.()) {
    ...
    check(
      "decrypted backup created with items",
      !!decBackup && decBackup.items.length > 0,
    );
    check(
      "decrypted backup EXCLUDES items keys (no key-material leak)",
      !decBackup.items.some((i: any) =>
        String(i.content_type).includes("ItemsKey"),
      ),
    );
    ...
    check(
      ...
      uuids.every(
        (u) => bNotes.find((r: any) => r.uuid === u)?.text === bodies[u],
      ),
    );
