...
async function main(): Promise<void> {
  ...
  const getBan = await fetch(
    `${base()}/v1/admin/users/${encodeURIComponent(email)}/ban-status`,
    {
  ...
  check(
    "a non-admin cannot read ban status (admin gate returns 401)",
    getBan.status === 401,
  );
  ...
  const ownUuid: string | undefined =
    app.app.getUser?.()?.uuid ?? app.app.sessions.getUser?.()?.uuid;
  const targetUuid = ownUuid ?? "00000000-0000-0000-0000-000000000000";
  const setBan = await fetch(
    `${base()}/v1/admin/users/${targetUuid}/ban-status`,
    {
      method: "PUT",
      headers: {
  ...
  check(
    "a non-admin cannot set ban status (admin gate returns 401)",
    setBan.status === 401,
  );
  ...
  check(
    "the ordinary account session is unaffected by the rejected admin calls",
