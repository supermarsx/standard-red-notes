...
async function main(): Promise<void> {
  ...
  await appB.sync();
  const pulled = uuids.filter((u) =>
    appB.app.items
      .getDisplayableNotes()
      .some((n: { uuid: string }) => n.uuid === u),
  );
  ...
  const originalsPresent = uuids.filter((u) =>
    notesC.some((n) => n.uuid === u),
  );
  check(
    "third device: all 5 original notes still present (none vanished)",
    originalsPresent.length === 5,
  );

  const conflictCopies = notesC.filter(
    (n) => n.conflictOf === uuids[1] || n.conflictOf === uuids[2],
  );
  check(
    "third device: exactly 2 conflict copies materialized (for the 2 overlapping edits)",
    conflictCopies.length === 2,
  );

  check(
    "third device: EXACT final count reconciles to 7 (5 originals + 2 conflict copies)",
    notesC.length === 7,
  );
  ...
  console.log(
    "  info - third-device bodies:",
    JSON.stringify(bodies.slice().sort()),
  );
  check(
    "third device: every edit from BOTH devices survives (no lost edit)",
    missing.length === 0,
  );
  if (missing.length) {
