...
async function main(): Promise<void> {
  ...
  const notesOnD2 = app2.app.items
    .getDisplayableNotes()
    .filter(
  ...
  check(
    "device 2 now has two copies of the note (conflict materialized)",
    notesOnD2.length >= 2,
  );

  const bodies = notesOnD2.map((n: { text?: string }) => n.text ?? "");
  check(
    "both edited versions survive (no data loss)",
    bodies.includes("edited by device 1") &&
      bodies.includes("edited by device 2"),
  );
  const hasConflictLink = notesOnD2.some(
    (n: { conflictOf?: string }) => n.conflictOf === created.uuid,
  );
  check(
    "the conflict copy is linked to the original via conflict_of",
    hasConflictLink,
  );
