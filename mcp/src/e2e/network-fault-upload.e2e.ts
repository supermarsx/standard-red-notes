...
globalThis.fetch = ((input: any, init?: any) => {
  ...
  if (faultActive && String(url).includes(serverHost)) {
    return Promise.reject(new TypeError('injected network fault: fetch failed'))
  }
...
async function main(): Promise<void> {
  ...
  const base = await clientA.createNote({ title: 'Base', body: 'synced baseline', tags: [] })
  await A.app.sync()
  ...
  await appB.sync()
  const clientB = new SnjsBackedClient(appB, { allowWrites: true, baseUrl: SERVER })
  check('device B pulled the baseline note', (await clientB.listNotes(50)).notes.some((n) => n.uuid === base.uuid))
  ...
  await appA.mutator.changeItem(
    appA.items.getDisplayableNotes().find((n: { uuid: string }) => n.uuid === base.uuid),
    (m: { text: string }) => { m.text = 'edited during outage' },
  )
  const newNote1 = await appA.mutator.createItem(
    (await import('@standardnotes/snjs')).default.ContentType.TYPES.Note,
    { title: 'Outage-1', text: 'authored during outage', references: [] }, true,
  )
  const newNote2 = await appA.mutator.createItem(
    (await import('@standardnotes/snjs')).default.ContentType.TYPES.Note,
    { title: 'Outage-2', text: 'authored during outage', references: [] }, true,
  )
  ...
  const blocked = await globalThis.fetch(`${SERVER}/healthcheck`).then(() => false).catch(() => true)
  check('network fault genuinely blocks the sync origin (outage is real)', blocked === true)
  ...
  check('outage syncs did NOT silently clear dirty items (no false success)', dirtyCount(A.app) >= 3)
  const dirtyUuids = new Set((appA.items.getDirtyItems?.() ?? []).map((i: { uuid: string }) => i.uuid))
  check('the edited baseline note is STILL dirty (retained, not dropped)', dirtyUuids.has(base.uuid))
  check('outage-authored note 1 is STILL dirty (retained)', dirtyUuids.has(newNote1.uuid))
  check('outage-authored note 2 is STILL dirty (retained)', dirtyUuids.has(newNote2.uuid))
  check('all local changes survive the outage locally', appA.items.getDisplayableNotes().some((n: { uuid: string; text?: string }) => n.uuid === base.uuid && n.text === 'edited during outage'))
  ...
  check('server never received the outage edit (device B does not see it yet)',
  ...
  check('server still holds the pre-outage baseline body (edit did not leak)', bBase?.body === 'synced baseline')
  ...
  check('after reconnect device A uploaded everything (nothing dirty)', dirtyCount(A.app) === 0)

  let converged = false
  for (let i = 0; i < 25; i++) {
    await appB.sync()
    const notes = (await clientB.listNotes(200)).notes
    const editArrived = (await clientB.readNote(base.uuid).catch(() => undefined))?.body === 'edited during outage'
    if (notes.some((n) => n.uuid === newNote1.uuid) && notes.some((n) => n.uuid === newNote2.uuid) && editArrived) {
      converged = true
      break
    }
    await sleep(300)
  }
  check('after reconnect device B converges on ALL retained changes (no loss)', converged)
