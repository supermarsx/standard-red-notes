...
async function main(): Promise<void> {
  ...
  const created = await call("notes.create", {
    title: "MCP e2e note",
    body: "hello over stdio",
    tags: ["proto"],
  });
  const uuid = created.uuid as string;
  check(
    "notes.create returns a uuid",
    typeof uuid === "string" && uuid.length > 0,
  );

  const read = await call("notes.read", { uuid });
  check(
    "notes.read returns the note body + tag",
    read.body === "hello over stdio" &&
      (read.tags as string[]).includes("proto"),
  );

  const list = await call("notes.list", { limit: 50 });
  check(
    "notes.list includes the created note",
    (list.notes as { uuid: string }[]).some((n) => n.uuid === uuid),
  );

  const search = await call("notes.search", { query: "stdio", limit: 10 });
  check(
    "notes.search finds it by body",
    (search.hits as { uuid: string }[]).some((h) => h.uuid === uuid),
  );

  const updated = await call("notes.update", {
    uuid,
    body: "edited over stdio",
    tags: ["edited"],
  });
  check(
    "notes.update returns uuid + updatedAt",
    updated.uuid === uuid && typeof updated.updatedAt === "string",
  );
  const reread = await call("notes.read", { uuid });
  check(
    "notes.update persisted the new body + tag",
    reread.body === "edited over stdio" &&
      (reread.tags as string[]).includes("edited"),
  );

  const tags = await call("tags.list");
  check(
    "tags.list includes the note tags",
    (tags.tags as { title: string }[]).some((t) => t.title === "proto"),
  );

  const vault = await call("vaults.create", { name: "MCP Vault" });
  const vaultUuid = vault.uuid as string;
  check(
    "vaults.create returns a uuid",
    typeof vaultUuid === "string" && vaultUuid.length > 0,
  );
  const vlist = await call("vaults.list");
  check(
    "vaults.list includes the new vault",
    (vlist.vaults as { uuid: string }[]).some((v) => v.uuid === vaultUuid),
  );
  const vnote = await call("notes.create", {
    title: "In vault",
    body: "v",
    tags: [],
    vault: vaultUuid,
  ...
  check(
    "note created into a vault reads back in that vault",
    vread.vault === "MCP Vault",
  );
  ...
  check(
    "deleted note no longer listed",
    !(afterDelete.notes as { uuid: string }[]).some((n) => n.uuid === uuid),
  );
  ...
  check(
    "write tool is rejected when writes are disabled",
