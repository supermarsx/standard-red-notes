...
async function syncUntilStable(
  app: HeadlessApp,
  label: string,
): Promise<number> {
  let prev = -1;
  for (let i = 0; i < 40; i++) {
    try {
      ...
      console.log(
        `  ..  ${label} sync attempt ${i} errored: ${(e as Error).message}`,
      );
    }
...
async function main(): Promise<void> {
  ...
  check(
    `authored ${N} distinct notes locally`,
    new Set(createdUuids).size === N,
  );
  ...
  check(
    "device A uploaded all notes (nothing left dirty)",
    dirtyCount(A.app) === 0,
  );
  ...
  check(
    `no note SKIPPED across pagination (missing=${missing.length})`,
    missing.length === 0,
  );
  check(
    `no UNEXPECTED note appeared (extra=${extra.length})`,
    extra.length === 0,
  );
  check(
    "device B uuid-set EXACTLY equals device A authored set",
    bSet.size === createdSet.size && missing.length === 0 && extra.length === 0,
  );
  ...
  check(
    "a second sync is a no-op (sync-token coherent across pages)",
    bCount2 === N,
  );
  ...
  for (const uuid of editUuids) {
    const note = appA.items
      .getDisplayableNotes()
      .find((n: { uuid: string }) => n.uuid === uuid);
  ...
  check(
    "device A uploaded the edited slice (nothing dirty)",
    dirtyCount(A.app) === 0,
  );
  ...
  for (let i = 0; i < 30; i++) {
    ...
    editsSeen = editUuids.filter((uuid) => {
      const n = appB.app.items
        .getDisplayableNotes()
        .find((x: { uuid: string }) => x.uuid === uuid);
  ...
  check(
    `device B converged on all ${EDIT_SLICE} edits`,
    editsSeen === EDIT_SLICE,
  );
  check(
    `device B still holds exactly ${N} notes after edits (no phantom loss/dup)`,
    appB.app.items.getDisplayableNotes().length === N,
  );
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const anyDeletedStillPresent = noteUuids(appB).some((u) =>
      deleteSet.has(u),
    );
  ...
  check(
    `device B converged to ${N - DELETE_SLICE} notes after delete (got ${bAfterDelete})`,
    bAfterDelete === N - DELETE_SLICE,
  );
  check(
    "none of the deleted notes remain on device B",
    !deleteUuids.some((u) => bUuidsFinal.has(u)),
  );
  check(
    "all still-live notes remain on device B (no collateral loss)",
    createdUuids.slice(0, N - DELETE_SLICE).every((u) => bUuidsFinal.has(u)),
  );
