...
async function main(): Promise<void> {
  ...
  const unsetEmpty200 =
    unset.status === 200 &&
    unset.body?.success === true &&
    unset.body?.setting === undefined;
  const unsetLegacy404 =
    unset.status === 400 &&
    /not found/i.test(String(unset.body?.error?.message ?? ""));
  if (unsetLegacy404) {
    skip(
      "unset optional setting returns empty 200",
      "deployed build still 400s on a missing setting (pre-fork)",
    );
  } else {
    check(
      "reading an unset optional setting returns an empty 200 (no value)",
      unsetEmpty200,
    );
  }
  check(
    "an unset optional setting never carries a value",
    unset.body?.setting === undefined,
  );
  ...
  const setWeekly = await putSetting(
    uuid,
    token,
  ...
  check(
    "EMAIL_BACKUP_FREQUENCY persisted as weekly",
    readWeekly.body?.setting?.value === "weekly",
  );

  // --- Update over an existing value (idempotent re-set), then read back. -----
  const setDisabled = await putSetting(
    uuid,
    token,
    "EMAIL_BACKUP_FREQUENCY",
    "disabled",
  ...
  check(
    "re-setting EMAIL_BACKUP_FREQUENCY to disabled succeeds",
    setDisabled.status === 200,
  );
  const readDisabled = await getSetting(uuid, token, "EMAIL_BACKUP_FREQUENCY");
  check(
    "EMAIL_BACKUP_FREQUENCY updated to disabled",
    readDisabled.body?.setting?.value === "disabled",
  );
  ...
  check(
    "after delete the setting carries no value",
    afterDelete.body?.setting === undefined,
  );
  check(
    ...
    (afterDelete.status === 200 && afterDelete.body?.success === true) ||
      (afterDelete.status === 400 &&
        /not found/i.test(String(afterDelete.body?.error?.message ?? ""))),
  );
  ...
  check(
    "setting an unknown setting name is rejected (400)",
    bogus.status === 400,
  );
