...
const liveNotes = (app: HeadlessApp): string[] =>
  app.app.items.getDisplayableNotes().map((n: { uuid: string }) => n.uuid);

function dockerRestart(): void {
  console.log(`  ..  docker restart ${CONTAINER}`);
  execFileSync("docker", ["restart", "-t", "2", CONTAINER], {
    stdio: "ignore",
  });
}
...
async function waitHealthy(label: string, timeoutMs = 90000): Promise<boolean> {
  ...
  console.log(
    `  ..  ${label}: server did NOT return healthy within ${timeoutMs}ms`,
  );
  return false;
}
...
async function main(): Promise<void> {
  ...
  if (
    !/localhost:3011$/.test(new URL(SERVER).host) &&
    !process.env.E2E_RESTART_FORCE
  ) {
    console.log(
      `SKIP: refusing to restart a non-isolated server (${SERVER}); set E2E_RESTART_FORCE to override`,
    );
    process.exit(0);
  }
  ...
  console.log(
    `  ..  after mid-upload crash: ${dirtyAfterCrash} dirty, ~${uploadedBeforeCrash} already uploaded`,
  );
  ...
  for (let i = 0; i < 80 && dirtyCount(A.app) > 0; i++) {
    try {
      ...
      /* keep retrying through reconnect */
    }
    await sleep(250);
  }
  check(
    "after recovery device A uploaded everything (nothing dirty)",
    dirtyCount(A.app) === 0,
  );
  ...
  check(
    `device B has EXACTLY ${N} notes after upload-crash recovery (got ${bUuids.length})`,
    bUuids.length === N,
  );
  check(
    "device B has NO duplicate notes (set size == array length)",
    bSet.size === bUuids.length,
  );
  check(
    "device B lost NO authored note",
    createdUuids.every((u) => bSet.has(u)),
  );
  check(
    "device B has NO note the author never created",
    bUuids.every((u) => createdSet.has(u)),
  );

  // === PHASE 2: crash mid-DOWNLOAD (cold-load completeness) ===
  const dirC = await fs.mkdtemp(path.join(os.tmpdir(), "srn-restart-C-"));
  const appC = await bootstrapHeadlessApp({
    serverUrl: SERVER,
    dataDir: dirC,
    password: A.password,
    syncIntervalMs: 0,
  ...
  const signInPromise = appC.signIn(A.email, A.password).catch((e: Error) => {
    console.log(
      `  ..  device C sign-in rejected (expected if crash landed mid-flight): ${e.message}`,
    );
  });
  ...
  console.log(
    `  ..  device C after interrupted download: ${cPartial} notes, completedOnlineDownloadFirstSync=${completedFlag}`,
  );
  ...
  check(
    "interrupted download NOT falsely reported complete (no partial-as-clean)",
    !(completedFlag && cPartial < N),
  );

  check(
    "server recovered healthy after phase-2 restart",
    await waitHealthy("phase2"),
  );
  ...
  for (let i = 0; i < 80; i++) {
    try {
      ...
      /* retry through reconnect */
    }
    cUuids = liveNotes(appC);
    if (
      cUuids.length >= N &&
      appC.app.sync.completedOnlineDownloadFirstSync === true
    )
      break;
    await sleep(250);
  }
  ...
  check(
    `device C converged to EXACTLY ${N} notes after download-crash (got ${cUuids.length})`,
    cUuids.length === N,
  );
  check("device C has NO duplicate notes", cSet.size === cUuids.length);
  check(
    "device C lost NO note",
    createdUuids.every((u) => cSet.has(u)),
  );
  check(
    "device C download-first sync is now genuinely complete",
    appC.app.sync.completedOnlineDownloadFirstSync === true,
  );
