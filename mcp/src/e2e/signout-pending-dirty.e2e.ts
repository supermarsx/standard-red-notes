...
async function main(): Promise<void> {
  ...
  await appA.mutator.changeItem(
    appA.items
      .getDisplayableNotes()
      .find((n: { uuid: string }) => n.uuid === note.uuid),
    (m: { text: string }) => {
  ...
  check(
    "there is a pending dirty (unsynced) item before sign-out",
    dirty.some((i: { uuid: string }) => i.uuid === note.uuid),
  );
  ...
  check(
    "sign-out with dirty items RAISED the data-loss warning (confirm called)",
    confirmCalls === 1,
  );
  check(
    "the warning text names the unsynced-change loss",
    /unsynced|lost forever/i.test(lastConfirmText),
  );
  ...
  const appF = await bootstrapHeadlessApp({
    serverUrl: SERVER,
    dataDir: dirF,
    password: A.password,
    syncIntervalMs: 0,
  ...
  check(
    "fresh device shows the SYNCED base body — the unsynced edit was DROPPED, not flushed",
    fresh?.text === "synced-base",
  );
  if (fresh?.text === "UNSYNCED-edit-should-be-dropped") {
    console.log(
      "  info - NOTE: the unsynced edit was FLUSHED before sign-out (would contradict the source contract).",
    );
  } else {
    console.log(
      "  info - VERDICT: contract confirmed — snjs WARNS then DROPS unsynced dirty items on non-forced sign-out (no flush).",
    );
  }
  ...
  const clean = await clientB.createNote({
    title: "Clean",
    body: "clean-synced-body",
    tags: [],
  });
  await B.app.sync();
  const noDirty = B.app.app.items.getDirtyItems?.() ?? [];
  check(
    "after a full sync there are no dirty items pending",
    noDirty.length === 0,
  );
  ...
  check(
    "clean sign-out (no dirty items) raises NO data-loss warning",
    bConfirmCalls === 0,
  );
  ...
  const appB2 = await bootstrapHeadlessApp({
    serverUrl: SERVER,
    dataDir: dirB2,
    password: B.password,
    syncIntervalMs: 0,
  ...
  for (let i = 0; i < 8; i++) {
    await appB2.sync();
    cleanFresh = appB2.app.items
      .getDisplayableNotes()
      .find((n: { uuid: string }) => n.uuid === clean.uuid);
    if (cleanFresh) break;
    await sleep(500);
  }
  check(
    "after a clean sign-out + re-sign-in, the synced note is intact (no loss)",
    cleanFresh?.text === "clean-synced-body",
  );
