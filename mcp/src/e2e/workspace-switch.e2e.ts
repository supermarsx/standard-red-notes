...
async function bridgeSignIn(
  ...
  try {
    ...
    const response = await app.app.signIn(
      email,
      password,
      false,
      false,
...
async function main(): Promise<void> {
  ...
  check(
    "the SAME email registered under two different workspaces (A=default, B) — both succeed",
    flagOn,
  );
  ...
  check(
    "sign-in to workspace B with the same email succeeds (switch workspace)",
    devB.ok,
  );
  ...
  if (devB.ok) {
    ...
    const clientB = new SnjsBackedClient(devB.app, {
      allowWrites: true,
      baseUrl: SERVER,
    });
    ...
    const noteB = await clientB.createNote({
      title: "B-only",
      body: "lives in workspace B",
      tags: ["wsB"],
    });
    await devB.app.sync().catch(() => {});
    await a.app.sync().catch(() => {});
    ...
    check(
      "workspace A still sees its own note after the switch",
      listInA.notes.some((n) => n.uuid === noteA.uuid),
    );
  ...
  check(
    "key-params for a KNOWN (email, workspace) returns 200",
    knownKp.status === 200,
  );
  check(
    "key-params for an UNKNOWN (email, workspace) ALSO returns 200 (pseudo params, no 404)",
    ghostKp.status === 200,
  );
  check(
