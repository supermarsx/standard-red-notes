...
async function ensureSelfContact(headless: {
  app: unknown;
  sync: () => Promise<void>;
}): Promise<boolean> {
  const app = headless.app as {
    contacts: {
      getSelfContact(): unknown;
      createOrEditTrustedContact(p: Record<string, unknown>): Promise<unknown>;
    };
    sessions: {
      getSureUser(): { uuid: string };
      getPublicKey(): string;
...
async function acceptInvite(
  ...
  const record = await pollFor("B inbound invite", async () => {
    ...
    return records.find(
      (r: { invite: { shared_vault_uuid: string } }) =>
        r.invite.shared_vault_uuid === sharedVaultUuid,
    );
  });
...
async function main(): Promise<void> {
  ...
  const clientB = new SnjsBackedClient(B.app, {
    allowWrites: true,
    baseUrl: SERVER,
  });
  ...
  const trustedB = await appA.contacts.addTrustedContactFromCollaborationID(
    collabB,
    "Human",
  );
  const trustedA = await appB.contacts.addTrustedContactFromCollaborationID(
    collabA,
    "Agent",
  );
  ...
  const vault = await appA.vaults.createRandomizedVault({
  ...
  const invite1 = await appA.vaultInvites.inviteContactToSharedVault(
    shared,
    trustedB,
    "write",
  );
  ...
  const note1 = await clientA.createNote({
    title: "Original",
    body: "first shared note",
    tags: [],
    vault: shared.uuid,
  });
  await A.app.sync();
  ...
  const seen1 = await pollFor("B sees note1", async () => {
    await B.app.sync();
    return (await clientB.listNotes(100)).notes.find(
      (n) => n.uuid === note1.uuid,
    );
  });
  ...
  const membersBefore =
    await appA.vaultUsers.getSharedVaultUsersFromServer(shared);
  check(
    "server member list includes B before leaving",
    Array.isArray(membersBefore) && membersBefore.length >= 2,
  );
  const bServerUser = (membersBefore ?? []).find(
    (u: { user_uuid: string }) =>
      u.user_uuid === appB.sessions.getSureUser().uuid,
  );
  ...
  const findBySharedUuid = (app: any) =>
    app.vaults
      .getVaults()
      .find((v: any) => v.sharing?.sharedVaultUuid === sharedVaultUuid);
  ...
  check(
    "leaveSharedVault did not return a displayable error",
    !leaveRes ||
      leaveRes.isFailed === undefined ||
      leaveRes.isFailed?.() === false,
  );
  await B.app.sync();
  await sleep(1500);
  await B.app.sync();
  ...
  const bSeesNoteAfterLeave = (await clientB.listNotes(200)).notes.some(
    (n) => n.uuid === note1.uuid,
  );
  check(
    "B can no longer see the shared note after leaving",
    !bSeesNoteAfterLeave,
  );
  ...
  const aSeesNoteAfterLeave = (await clientA.listNotes(200)).notes.some(
    (n) => n.uuid === note1.uuid,
  );
  check(
    "A still has the vault and note after B left",
    aHasVaultAfterLeave && aSeesNoteAfterLeave,
  );
  ...
  const invite2 = await appA.vaultInvites.inviteContactToSharedVault(
    sharedRefetch,
    trustedB,
    "write",
  );
  ...
  const note2 = await clientA.createNote({
    title: "Added while gone",
    body: "written before B rejoined",
    tags: [],
    vault: shared.uuid,
  });
  await A.app.sync();
  ...
  const seenOld = await pollFor("B re-sees note1 after rejoin", async () => {
    await B.app.sync();
    return (await clientB.listNotes(200)).notes.find(
      (n) => n.uuid === note1.uuid,
    );
  });
  ...
  // === (3) A (owner) REMOVES B ===
  check(
    "A is the vault owner",
    appA.vaultUsers.isCurrentUserSharedVaultOwner(sharedRefetch) === true,
  );
  const removeRes = await appA.vaultUsers.removeUserFromSharedVault(
    sharedRefetch,
    appB.sessions.getSureUser().uuid,
  );
  check(
    "owner removeUserFromSharedVault succeeded",
    removeRes?.isFailed?.() === false,
  );
  await A.app.sync();
  await sleep(1500);

  const membersAfterRemove =
    await appA.vaultUsers.getSharedVaultUsersFromServer(sharedRefetch);
  const bMemberAfterRemove = (membersAfterRemove ?? []).some(
    (u: { user_uuid: string }) =>
      u.user_uuid === appB.sessions.getSureUser().uuid,
  );
  check(
    "server member list no longer includes B after owner-remove",
    !bMemberAfterRemove,
  );
  ...
  check(
    "B no longer has the vault after being removed by the owner",
    !bHasVaultAfterRemove,
  );
  ...
  check(
    "A still owns the vault and its notes after removing B",
    aIntactAfterRemove,
  );
