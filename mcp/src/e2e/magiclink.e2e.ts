...
async function main(): Promise<void> {
  ...
  check(
    "magic-link status endpoint reports enabled",
    await magicLinkEnabled(tokenA!),
  );
  ...
  const reqBody = (await reqRes.json().catch(() => ({}))) as {
    code?: string;
    data?: { code?: string };
  };
  const onScreen = reqBody.code ?? reqBody.data?.code;
  check(
    "magic-link request returns an on-screen code (SMTP fallback)",
    typeof onScreen === "string" && /^\d{4,8}$/.test(onScreen ?? ""),
  );
  ...
  check(
    "sign-in actually went through an MFA challenge (enforced)",
    dev.mfaChallenges >= 1,
  );
  if (dev.ok) {
    await dev.app.sync();
    const c2 = new SnjsBackedClient(dev.app, {
      allowWrites: true,
      baseUrl: SERVER,
    });
    const read = await c2.readNote(note.uuid).catch(() => undefined);
    check(
      "the signed-in device can decrypt the guarded note",
      read?.body === "guarded by magic-link",
    );
    await setMagicLink(accessToken(dev.app)!, false);
    check(
      "magic-link status endpoint reports disabled",
      (await magicLinkEnabled(accessToken(dev.app)!)) === false,
    );
  }
  ...
  check(
    "sign-in after disabling needs no MFA challenge",
    plain.ok && plain.mfaChallenges === 0,
  );
