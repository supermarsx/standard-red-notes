...
async function main(): Promise<void> {
  ...
  check(
    "POST /v1/email-reminders creates a reminder (200 + id)",
    created.status === 200 && Boolean(reminderId),
  );
  ...
  if (reminderId) {
    ...
    check(
      "DELETE /v1/email-reminders removes the reminder (200)",
      delRes.status === 200,
    );
  ...
  for (let i = 0; i < MAX_ATTEMPTS; i++) {
    ...
    limitMessage = String(
      r.data?.error?.message ?? r.data?.data?.error?.message ?? "",
    );
    break;
  }
