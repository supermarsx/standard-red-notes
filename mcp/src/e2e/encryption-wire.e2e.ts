...
const PLAINTEXT_BODY =
  "PLAINTEXT-SECRET-BODY-must-never-appear-on-the-wire-4242";
...
async function main(): Promise<void> {
  ...
  const created = await clientA.createNote({
  ...
  check(
    ...
    typeof notePayload?.content === "string" &&
      notePayload.content.startsWith("004:"),
  );
  check(
    "persisted enc_item_key is 004 ciphertext (004: prefix)",
    typeof notePayload?.enc_item_key === "string" &&
      notePayload.enc_item_key.startsWith("004:"),
  );
  const localBlob = JSON.stringify(payloads);
  check(
    "plaintext BODY does not appear anywhere in the local db",
    !localBlob.includes(PLAINTEXT_BODY),
  );
  check(
    "plaintext TITLE does not appear anywhere in the local db",
    !localBlob.includes(PLAINTEXT_TITLE),
  );
  const itemsKeyPayload = payloads.find((p) =>
    String(p.content_type).includes("ItemsKey"),
  );
  check("an items key is persisted locally", !!itemsKeyPayload);
  check(
    "the items key itself is root-key-encrypted (004: enc_item_key)",
    typeof itemsKeyPayload?.enc_item_key === "string" &&
      itemsKeyPayload.enc_item_key.startsWith("004:"),
  );
  ...
  if (!token) {
    ...
    const retrieved: any[] =
      body?.retrieved_items ?? body?.data?.retrieved_items ?? [];
    ...
    check(
      ...
      typeof serverNote?.content === "string" &&
        serverNote.content.startsWith("004:"),
    );
    const wireBlob = JSON.stringify(retrieved);
    check(
      "plaintext BODY does not appear in any server payload",
      !wireBlob.includes(PLAINTEXT_BODY),
    );
    check(
      "plaintext TITLE does not appear in any server payload",
      !wireBlob.includes(PLAINTEXT_TITLE),
    );
  }
  ...
  const good = await bootstrapHeadlessApp({
    serverUrl: SERVER,
    dataDir: dirGood,
    password: A.password,
    syncIntervalMs: 0,
  ...
  await sleep(200);
  await good.sync();
  const goodNote = good.app.items
    .getDisplayableNotes()
    .find((n: any) => n.uuid === created.uuid);
  check("fresh device with the correct password sees the note", !!goodNote);
  check(
    "decrypted title matches the original",
    goodNote?.title === PLAINTEXT_TITLE,
  );
  check(
    "decrypted body matches the original",
    goodNote?.text === PLAINTEXT_BODY,
  );
  check(
    "no item is left errorDecrypting on the good device",
    (good.app.items.invalidItems ?? []).length === 0,
  );
  ...
  check(
    "no notes are present on the failed-auth device (no partial leak)",
    bad.app.items.getDisplayableNotes().length === 0,
  );
  console.log("    wrong-password error:", errMsg.slice(0, 140));

  // ---- 5. The ciphertext is untouched: the good device still decrypts it ----
  await good.sync();
  const goodNote2 = good.app.items
    .getDisplayableNotes()
    .find((n: any) => n.uuid === created.uuid);
  check(
    "note still intact + decryptable after the failed-auth attempt",
    goodNote2?.text === PLAINTEXT_BODY,
  );
