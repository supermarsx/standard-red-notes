...
export class NodeDevice {
  ...
  private async ensureLoaded(): Promise<void> {
    ...
    this.keychain = (await this.readJson("keychain.json")) as Record<
      string,
      NamespacedRootKeyInKeychain
    >;
    const dbObj = (await this.readJson("db.json")) as Record<
      string,
      TransferPayload
    >;
  ...
  async getJsonParsedRawStorageValue(
    key: string,
  ...
  async removeRawStorageValuesForIdentifier(
  ...
  async getDatabaseLoadChunks(
    ...
    const remainingChunks: DatabaseFullEntryLoadChunk[] = [
      { entries: sorted.contentTypePriorityPayloads },
    ];
    for (
      let i = 0;
      i < sorted.remainingPayloads.length;
      i += options.batchSize
    ) {
    ...
    return {
      ...
      remainingChunksItemCount:
        sorted.contentTypePriorityPayloads.length +
        sorted.remainingPayloads.length,
    };
  }

  async getAllDatabaseEntries<
    T extends FullyFormedTransferPayload = FullyFormedTransferPayload,
  >(_identifier: ApplicationIdentifier): Promise<T[]> {
    await this.ensureLoaded();
  ...
  async getDatabaseEntries<
    T extends FullyFormedTransferPayload = FullyFormedTransferPayload,
  >(_identifier: ApplicationIdentifier, keys: string[]): Promise<T[]> {
    await this.ensureLoaded();
    return keys
      .map((k) => this.db.get(k))
      .filter((v): v is TransferPayload => v != null) as unknown as T[];
  }

  async saveDatabaseEntry(
  ...
  async removeAllDatabaseEntries(
    _identifier: ApplicationIdentifier,
  ): Promise<void> {
    await this.ensureLoaded();
    this.db.clear();
  ...
  async clearAllDataFromDevice(
    _workspaceIdentifiers: ApplicationIdentifier[],
  ): Promise<{ killsApplication: boolean }> {
    this.storage.clear();
    this.keychain = {};
    this.db.clear();
    await Promise.all([
