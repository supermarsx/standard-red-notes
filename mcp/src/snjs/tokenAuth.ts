...
export function parseFullToken(fullToken: string): {
  ...
  return {
    ...
    authSecret,
    wrapSecret,
    authToken: `${tokenUuid}.${authSecret}`,
  };
}
...
async function authenticate(
  ...
  if (res.status !== 200) {
    const message =
      (body as any)?.data?.error?.message ??
      body?.error?.message ??
      `HTTP ${res.status}`;
...
export function unwrapItemsKeys(
  ...
  const wrapKey: string = crypto.argon2(
    wrapSecret,
    keyMaterial.kdfSalt,
    iterations,
    bytes,
  ...
  const plaintext: string | null = crypto.xchacha20Decrypt(
    ciphertext,
    nonce,
    wrapKey,
  );
  if (plaintext === null) {
    throw new Error(
      "Failed to decrypt wrapped items keys (bad wrap secret or corrupt material).",
    );
  }

  const payload = JSON.parse(plaintext) as {
    v: number;
    itemsKeys: UnwrappedItemsKey[];
  };
 ...
 * re-uploaded.
 */
async function injectItemsKeys(
  app: any,
  itemsKeys: UnwrappedItemsKey[],
): Promise<void> {
  const payloads = itemsKeys.map(
    (key, index) =>
  ...
  await app.mutator.emitItemsFromPayloads(
    payloads,
...
export async function signInWithMcpToken(
  ...
  const accessTokenOrError = SessionToken.create(
    ...
    auth.session.access_expiration,
  );
  const refreshTokenOrError = SessionToken.create(
    auth.session.refresh_token,
    auth.session.refresh_expiration,
  );
  if (accessTokenOrError.isFailed() || refreshTokenOrError.isFailed()) {
    throw new Error(
      "Could not construct session tokens from MCP authenticate response.",
    );
  }
  const readOnly =
    auth.session.readonly_access === true || auth.mcp_scope?.access === "read";
  ...
  if (sessionOrError.isFailed()) {
    throw new Error(
      "Could not construct session from MCP authenticate response.",
    );
  }
  ...
  try {
    const keyRecovery = app.dependencies?.get?.(
      Symbol.for("KeyRecoveryService"),
    );
    keyRecovery?.deinit?.();
  } catch {
  ...
  try {
    if (!auth.mcp_key_material) {
      throw new Error(
        "MCP authenticate returned no key material; cannot decrypt notes.",
      );
    }
  ...
  await app.sync
    .sync({ sourceDescription: "mcp-token-post-inject" })
    .catch(() => {});
