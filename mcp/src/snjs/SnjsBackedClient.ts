...
export class SnjsBackedClient {
  ...
  private tagsForNote(note: any): string[] {
    return (this.app.items.getSortedTagsForItem(note) ?? []).map(
      (t: any) => t.title,
    );
  }
  ...
  private async resolveTag(title: string): Promise<any> {
    const existing = this.app.items
      .getDisplayableTags()
      .find((t: any) => t.title === title);
    if (existing) {
      return existing;
    }
  ...
  async createNote(input: {
    ...
    for (const tagTitle of input.tags ?? []) {
      ...
      await this.app.mutator.addTagToNote(
        this.noteByUuid(note.uuid),
        tag,
        false,
      );
