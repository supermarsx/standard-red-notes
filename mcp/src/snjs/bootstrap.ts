...
const { SNApplication, SNLog, Platform, ApiVersion, CreateChallengeValue } =
...
export async function bootstrapHeadlessApp(
  ...
  async function fetchMagicLinkCode(
    email: string,
  ): Promise<string | undefined> {
    try {
      const res = await fetch(
        `${options.serverUrl.replace(/\/$/, "")}/v1/mfa/magic-link/request`,
        {
          method: "POST",
    ...
    receiveChallenge: (challenge: any) => {
      if (process.env.MCP_DEBUG_CHALLENGES) {
        ...
        console.error(
          ...
          challenge.reason,
          challenge.heading,
          JSON.stringify((challenge.prompts ?? []).map((p: any) => p.title)),
        );
      }
      ...
      const onScreenCode =
        heading.match(/code[^0-9]*?(\d{4,8})/i)?.[1] ??
        (codeRuns ? codeRuns[codeRuns.length - 1] : undefined);
      ...
      const values = (challenge.prompts ?? []).map((prompt: any) => {
        ...
        if (process.env.MCP_DEBUG_CHALLENGES) {
          // eslint-disable-next-line no-console
          console.error(
            "[snjs challenge] unrecognized prompt; submitting empty:",
            prompt.title,
          );
        }
  ...
  const result: HeadlessApp = {
    ...
    async register(email: string, pw: string): Promise<void> {
      ...
      if (response?.error) {
        throw new Error(
          `register failed: ${response.error.message ?? JSON.stringify(response.error)}`,
        );
      }
    },
    ...
    async signIn(email: string, pw: string, code?: string): Promise<void> {
      ...
      dynamicMfaCode =
        (code ?? mfaCode) ? undefined : await fetchMagicLinkCode(email);
      ...
      if (error) {
        ...
        if (/mfa|two.?factor|totp/i.test(message)) {
          throw new Error(
            `sign-in requires an MFA code; set STANDARD_RED_NOTES_MFA_CODE. (${message})`,
          );
        }
    ...
    isSignedIn(): boolean {
      ...
      return Boolean(
        app.hasAccount?.() ?? app.getUser?.() ?? app.sessions?.isSignedIn?.(),
      );
    },
