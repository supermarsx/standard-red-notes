...
server.registerTool(
  'notes.list',
  {
    ...
    inputSchema: {
      limit: z.number().int().positive().max(200).default(50).describe('Maximum number of notes to return.'),
      cursor: z.string().optional().describe('Opaque cursor returned by a previous call.'),
    },
    outputSchema: {
      notes: z.array(z.object({ uuid: z.string(), title: z.string(), updatedAt: z.string() })),
...
server.registerTool(
  ...
  {
    title: 'Search Notes',
    description: 'Search notes by title or body keywords. Returns matching note UUIDs, titles, and a snippet.',
    inputSchema: {
...
server.registerTool(
  ...
  {
    title: 'Read Note',
    description: 'Fetch a single note by UUID. Returns title, body, tags, and timestamps.',
    inputSchema: {
...
server.registerTool(
  ...
  async ({ title, body, tags }) => {
    if (!client.allowWrites) {
      throw new Error('Writes are disabled. Set STANDARD_RED_NOTES_ALLOW_WRITES=1 to enable notes.create.')
    }
...
server.registerTool(
  ...
  {
    title: 'Update Note',
    description: 'Update an existing note by UUID. Disabled unless STANDARD_RED_NOTES_ALLOW_WRITES=1.',
    inputSchema: {
      uuid: z.string().uuid(),
  ...
  async ({ uuid, title, body, tags }) => {
    if (!client.allowWrites) {
      throw new Error('Writes are disabled. Set STANDARD_RED_NOTES_ALLOW_WRITES=1 to enable notes.update.')
    }
...
server.registerTool(
  ...
  {
    title: 'Delete Note',
    description: 'Delete a note by UUID. Disabled unless STANDARD_RED_NOTES_ALLOW_WRITES=1.',
    inputSchema: { uuid: z.string().uuid() },
    outputSchema: { uuid: z.string(), deleted: z.boolean() },
  },
  async ({ uuid }) => {
    if (!client.allowWrites) {
      throw new Error('Writes are disabled. Set STANDARD_RED_NOTES_ALLOW_WRITES=1 to enable notes.delete.')
    }
